...
- Provides an offline-capable guided item/weapon/armor creator with all ten stat slots and all five item-spell slots, named class/subclass/quality/slot/binding choices, a live WotLK-style tooltip, SQL preview/export, and schema-aware transactional insertion when a server is connected. Its same-window display resolver follows the live SQL `displayid` through the real 25-field build-12340 `ItemDisplayInfo.dbc`, exposes every model/model-texture/icon/wear-texture/geoset/helmet/visual/particle/sound field, maps canonical client paths, finds every matching processed provenance source without a library-wide recursive scan, and loads a selected WotLK M2/SKIN directly. Items & Sets links straight to complete-row SQL Studio/Favorites and the byte-safe MPQ merge workspace.
