...
- Opens a native **Maps & World** workspace in the same window for WotLK ADT, WDT, and WDL files. It accepts file-picker, direct-open, CLI, and drag/drop input; validates reversed chunk headers and bounds; decodes WDT `MAIN` and WDL `MAOF` 64×64 presence grids; decodes every ADT `MCNK` 16×16 coordinate, area, holes, base height, and bounded `MCVT` height grid; inventories referenced terrain textures/models/WMOs; and provides a responsive clickable height-colored grid without launching or modifying files through Noggit.
