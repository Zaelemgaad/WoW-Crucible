...
- Creates portable content-project folders with separate Assets, DBC, SQL, Manifests, Reports, and Staging outputs plus a persistent ID registry. Verified reservations scan every mapped live SQL identity and matching DBC stable key before allocation, refuse incomplete coverage, enforce WotLK's 31-ID race/class ceiling, and share one collision namespace between mount and spell IDs. Manual occupied-ID lists remain available for unsupported/custom domains.
...
1. Surface the verified live SQL/DBC project-ID registry in the same-window guided editors and connect reservations to unified validation/change plans.
