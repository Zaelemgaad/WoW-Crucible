...
- Opens a native **Maps & World** workspace in the same window for WotLK ADT, WDT, and WDL files. It accepts file-picker, direct-open, CLI, and drag/drop input; validates reversed chunk headers and bounds; decodes WDT `MAIN` and WDL `MAOF` 64×64 presence grids; decodes every ADT `MCNK` 16×16 coordinate, area, holes, base height, and bounded `MCVT` height grid; inventories referenced terrain textures/models/WMOs; and provides a responsive clickable height-colored grid without launching Noggit. Single/Ctrl-multi/all-present terrain selection can preview a finite whole-cell height offset. True vertex brushes edit exact MCVT floats with click-positioned tile-local center/radius, raise/lower, absolute-height flatten, neighbor-aware smooth, repeatable seeded noise, and independent linear/smooth/constant falloff. Both paths write a separate hash-bound ADT atomically; the original is never an output target, affected cells are re-parsed, and portable receipts record exact pre/post identity.
