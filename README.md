...
- Adds native Wrath character animation sampling to the same embedded preview: version-264 sequences, alias chains, main-file/global tracks, external `.anim` payloads, packed quaternions, parented bone matrices, four-weight vertex skinning, animated attachment points, and animated bounds. The responsive in-window strip selects a sequence, plays/pauses, and scrubs exact milliseconds. It deliberately pauses while mounted item models are visible until child attachment following is implemented, rather than showing gear detached from an animated character; multi-pass shader blending and particles remain later fidelity stages.
