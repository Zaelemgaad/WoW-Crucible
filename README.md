...
- Decodes ADT/WDT WMO placement tables instead of collapsing them to filenames: MWID→MWMO name bindings, MODF unique IDs, finite position/orientation vectors, world extents, flags, doodad/name sets, and raw scale are validated and preserved. Maps & World lists every placed instance separately, resolves its exact client path through the provenance selector, and applies the selected instance's stored orientation and scale in the embedded WMO preview while showing its world position. Multi-instance world-scene composition remains a separate rendering stage.
