...
- Extends explicit build-264 material coverage with shaders 2, 5, and 23: base-alpha-masked environment composition, primary/primary additive-alpha, and primary/primary alpha interpolation now use bounded, testable pass plans. Real version-264 M2/SKIN specimens verify shaders 2 and 23 end to end; shader 5 has a synthetic structure regression because the encountered processed SKIN has no matching M2. These remain conservatively labeled approximate, and edge-fade shader 21 remains an honest first-stage fallback until view-angle fading is implemented.
