...
2. Extend **Legacy SQL Recovery & Promotion** beyond the implemented target conflict analysis, SQL preview, transactional apply, and receipt rollback: add graph-driven dependency-closure selection, cross-core field adapters, and collision-safe coordinated ID remapping. The default path remains read-only and never carries deletions into the target implicitly.
