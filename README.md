...
- Searches the live world schema plus the configured server `CharStartOutfit.dbc` for items with no known vendor, direct/reachable loot, usable linked starter→ender quest, SQL/DBC starting-item, prospecting, milling, disenchanting, fishing, or spell-loot acquisition path; it does not confuse nonzero loot-reference control rows with awarded item IDs, disabled quests, or orphaned quest-template rewards with playable acquisition. The result is searchable in the Avalonia item workbench, individual IDs have accepted/rejected evidence explanations, selected rows open directly in the decoded editor or favorites, and reports remain exportable from the CLI.
