...
- Decodes ADT/WDT object placement tables instead of collapsing them to filenames: MMID→MMDX/MDDF doodad M2s and MWID→MWMO/MODF world models preserve each unique ID, finite position/orientation, flags, and raw scale; WMO extents and doodad/name sets are retained as well. Maps & World lists every placed M2 and WMO instance separately, resolves its exact client path through the provenance selector, and opens the chosen native M2/SKIN or WMO geometry in the same preview tab. WMO orientation/scale are applied directly; M2 placement transforms are reported while multi-instance world-scene composition remains a separate rendering stage.
