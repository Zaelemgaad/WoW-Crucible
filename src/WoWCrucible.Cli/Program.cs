...
static async Task<int> Database(string[] args, CancellationToken cancellationToken)
{
    ...
    if (operation.Equals("sync-plan", StringComparison.OrdinalIgnoreCase))
    {
        ...
        var options = args[7..]; var includes = options.Where(option => option.StartsWith("--include=", StringComparison.OrdinalIgnoreCase)).Select(option => option[10..]).ToArray(); var removals = options.Any(option => option.Equals("--include-removals", StringComparison.OrdinalIgnoreCase)); var overwrite = options.Any(option => option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)); var maximumText = Option(options, "--maximum=") ?? "100000";
        var unknown = options.Where(option => !option.StartsWith("--password-env=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--ssl=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--include=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--maximum=", StringComparison.OrdinalIgnoreCase) && !option.Equals("--include-removals", StringComparison.OrdinalIgnoreCase) && !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown sync-plan option: {unknown[0]}"); if (!int.TryParse(maximumText, out var maximum) || maximum is < 1 or > 1_000_000) return Fail("--maximum must be from 1 through 1,000,000.");
        ...
        var result = await new DatabaseSynchronizationService().BuildPlanAsync(args[5], profile, args[6], new(includes, removals, maximum, overwrite), progress, cancellationToken); var plan = result.Plan;
