...
static int Asset(string[] args)
{
    ...
    {
        var json = dependencyOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var onlyProblems = dependencyOptions.Contains("--only-problems", StringComparer.OrdinalIgnoreCase); var manifestPath = Option(dependencyOptions, "--manifest="); var outputMpq = Option(dependencyOptions, "--output-mpq=") ?? "patch-Crucible-Assets.MPQ";
        var unknown = dependencyOptions.Where(option => !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase) && !option.Equals("--only-problems", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--manifest=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--output-mpq=", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown dependency-graph option: {unknown[0]}");
        var index = AssetComparisonService.BuildIndex(dependencyLibrary); var graph = ClientAssetDependencyService.Analyze(index, dependencyRoot);
        ...
        {
            Console.WriteLine($"ROOT\t{graph.Root.ClientPath}\t{graph.Root.Provenance}\t{graph.Root.SourcePath}\nNODES\t{graph.Nodes.Count}\nRESOLVED\t{graph.Resolved.Count}\nEXTERNAL_BINDINGS\t{graph.ExternalBindings.Count}\nBLOCKING\t{graph.Blocking.Count}");
            foreach (var node in graph.Nodes.Where(node => !onlyProblems || node.State is ClientAssetDependencyState.Missing or ClientAssetDependencyState.CrossSourceConflict or ClientAssetDependencyState.Invalid))
        ...
        if (manifestPath is not null)
        {
            ...
            PatchManifestService.Save(manifestPath, Path.GetFileNameWithoutExtension(manifestPath), outputMpq, graph.PatchEntries, policy: new(ExpectedEntryCount: graph.PatchEntries.Count)); Console.Error.WriteLine($"Wrote dependency-complete patch manifest with {graph.PatchEntries.Count:N0} file(s): {Path.GetFullPath(manifestPath)}");
        }
...
{
    if (args.Length > 0 && args[0] is "help" or "--help" or "-h") { Console.WriteLine("  wowcrucible dbc item-display <ItemDisplayInfo.dbc> <schema.xml|-> <display-id> [--class=N] [--subclass=N] [--inventory=N] [--assets=processed-library] [--format=text|json]\n  wowcrucible dbc item-equipped <ItemDisplayInfo.dbc> <schema.xml|-> <display-id> <base-skin.blp|image> <output.png> --inventory=N --assets=processed-library [--source=name] [--overwrite]"); return DbcHelp(); }
