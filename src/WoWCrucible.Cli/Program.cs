...
static async Task<int> Database(string[] args, CancellationToken cancellationToken)
{
    ...
    if (operation.Equals("sync-plan", StringComparison.OrdinalIgnoreCase))
    {
        ...
        var result = await new DatabaseSynchronizationService().BuildPlanAsync(args[5], profile, args[6], new(includes, removals, maximum, overwrite, autoRemap, remapStart, dependencyClosure), progress, cancellationToken); var plan = result.Plan;
