...
try
{
    ...
    devbug?.RecordCommand(commandArguments, requestedHelp);
    exitCode = commandArguments.Length == 0 ? Help() : requestedHelp is not null ? RoutedHelp(requestedHelp) : commandArguments[0].ToLowerInvariant() switch
    {
        ...
        "client" => Client(commandArguments[1..], cancellation.Token),
...
static async Task<int> Cache(string[] args, CancellationToken cancellationToken)
{
    ...
    WowCacheTable table;
    try { table = WowCacheTableService.LoadWdb(cachePath, definition); }
    catch (InvalidDataException exception) when (operation == "info")
    {
        var evidence = WowCacheTableService.InspectUnsupported(cachePath, exception.Message);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(evidence, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else Console.WriteLine($"Status\tUNSUPPORTED\nPath\t{evidence.SourcePath}\nBytes\t{evidence.Length:N0}\nSHA256\t{evidence.Sha256}\nFirstBytesHex\t{evidence.FirstBytesHex}\nFirstBytesAscii\t{evidence.FirstBytesAscii}\nFirstFourBytesHex\t{evidence.FirstFourBytesHex}\nRawMagic\t{evidence.RawMagicInterpretation}\nReversedMagic\t{evidence.ReversedMagicInterpretation}\nReason\t{evidence.Failure}");
        return 3;
    }
    if (operation == "info")
    {
...
static int Asset(string[] args, CancellationToken cancellationToken)
{
    ...
    if (args is ["wdt-create", var createWdtPath, .. var createWdtOptions])
    {
        var overwrite = createWdtOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var json = createWdtOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var tileText = Option(createWdtOptions, "--tiles=") ?? "none";
        var unknown = createWdtOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--tiles=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset wdt-create option: {unknown[0]}");
        IReadOnlyList<(int X, int Y)> cells = []; if (!tileText.Equals("none", StringComparison.OrdinalIgnoreCase) && !TryParseGridCells(tileText, 64, out cells)) return Fail("--tiles must use x:y,x:y within the 64x64 WDT grid, or none.");
        var result = WdtTileTableService.Create(createWdtPath, cells, overwrite);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(result, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else Console.WriteLine($"Output\t{result.OutputPath}\nSHA256\t{result.OutputSha256}\nReceipt\t{result.ReceiptPath}\nPresentTiles\t{result.Inspection.PresentCells:N0}"); return 0;
    }
    if (args is ["wdt-tiles-plan", var planWdtPath, var stateText, var wdtCellText, var wdtPlanPath, .. var wdtPlanOptions])
    {
        var overwrite = wdtPlanOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var json = wdtPlanOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase);
        var unknown = wdtPlanOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset wdt-tiles-plan option: {unknown[0]}");
        if (!stateText.Equals("present", StringComparison.OrdinalIgnoreCase) && !stateText.Equals("absent", StringComparison.OrdinalIgnoreCase)) return Fail("WDT tile state must be present or absent."); var present = stateText.Equals("present", StringComparison.OrdinalIgnoreCase); var inspection = MapAssetInspectionService.Inspect(planWdtPath); IReadOnlyList<(int X, int Y)> cells;
        if (wdtCellText.Equals("all", StringComparison.OrdinalIgnoreCase)) cells = inspection.Cells.Select(cell => (cell.X, cell.Y)).ToArray();
        else if (wdtCellText.Equals("all-missing", StringComparison.OrdinalIgnoreCase)) cells = inspection.Cells.Where(cell => !cell.Present).Select(cell => (cell.X, cell.Y)).ToArray();
        else if (wdtCellText.Equals("all-present", StringComparison.OrdinalIgnoreCase)) cells = inspection.Cells.Where(cell => cell.Present).Select(cell => (cell.X, cell.Y)).ToArray();
        else if (!TryParseGridCells(wdtCellText, 64, out cells)) return Fail("WDT tiles must use x:y,x:y, all, all-present, or all-missing.");
        var plan = WdtTileTableService.Plan(planWdtPath, cells, present); WdtTileTableService.SavePlan(plan, wdtPlanPath, overwrite);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(plan, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else Console.WriteLine($"Plan\t{Path.GetFullPath(wdtPlanPath)}\nInput\t{plan.InputPath}\nState\t{(plan.Present ? "present" : "absent")}\nRequestedTiles\t{plan.RequestedCells:N0}\nEditedTiles\t{plan.Edits.Count:N0}\nAlreadyDesired\t{plan.AlreadyDesiredCells:N0}\nInputSHA256\t{plan.InputSha256}"); return 0;
    }
    if (args is ["wdt-tiles-apply", var applyWdtPlanPath, var wdtOutputPath, .. var wdtApplyOptions])
    {
        var overwrite = wdtApplyOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var json = wdtApplyOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase);
        var unknown = wdtApplyOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset wdt-tiles-apply option: {unknown[0]}");
        var result = WdtTileTableService.Apply(WdtTileTableService.LoadPlan(applyWdtPlanPath), wdtOutputPath, overwrite);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(result, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else Console.WriteLine($"Output\t{result.OutputPath}\nSHA256\t{result.OutputSha256}\nReceipt\t{result.ReceiptPath}\nEditedTiles\t{result.EditedCells:N0}\nState\t{(result.Present ? "present" : "absent")}\nPresentTiles\t{result.Inspection.PresentCells:N0}"); return 0;
    }
    ...
    if (args is ["ground-effect-catalog", var groundDbcRoot, .. var groundCatalogOptions])
    {
        var json = groundCatalogOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var query = Option(groundCatalogOptions, "--query="); var limitText = Option(groundCatalogOptions, "--limit=") ?? "256"; var unknown = groundCatalogOptions.Where(option => !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--query=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--limit=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset ground-effect-catalog option: {unknown[0]}"); if (!int.TryParse(limitText, out var limit) || limit is < 1 or > 10_000) return Fail("--limit must be 1 through 10,000.");
        var catalog = GroundEffectCatalogService.Load(groundDbcRoot); var matches = GroundEffectCatalogService.Search(catalog, query, limit);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(new { catalog.DbcDirectory, catalog.TextureSha256, catalog.DoodadSha256, TotalEffects = catalog.Effects.Count, TotalDoodads = catalog.Doodads.Count, Query = query, Results = matches, catalog.Findings }, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else { Console.WriteLine($"DBC\t{catalog.DbcDirectory}\nEffects\t{catalog.Effects.Count:N0}\nDoodads\t{catalog.Doodads.Count:N0}\nMatches\t{matches.Count:N0}"); foreach (var effect in matches) Console.WriteLine($"EFFECT\t{effect.Id}\tdensity={effect.Density}\tsound={effect.Sound}\tdoodads={string.Join(',', effect.DoodadIds)}\tweights={string.Join(',', effect.DoodadWeights)}\tmodels={string.Join('|', effect.Doodads.Where(value => value is not null).Select(value => value!.ClientModelPath))}"); foreach (var finding in catalog.Findings) Console.WriteLine($"FINDING\t{finding}"); }
        return catalog.Findings.Count == 0 ? 0 : 3;
    }
    if (args is ["liquid-type-catalog", var liquidDbcRoot, .. var liquidCatalogOptions])
    {
        var json = liquidCatalogOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var query = Option(liquidCatalogOptions, "--query="); var limitText = Option(liquidCatalogOptions, "--limit=") ?? "256"; var unknown = liquidCatalogOptions.Where(option => !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--query=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--limit=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset liquid-type-catalog option: {unknown[0]}"); if (!int.TryParse(limitText, out var limit) || limit is < 1 or > 10_000) return Fail("--limit must be 1 through 10,000.");
        var catalog = LiquidTypeCatalogService.Load(liquidDbcRoot); var matches = LiquidTypeCatalogService.Search(catalog, query, limit);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(new { catalog.DbcDirectory, catalog.Sha256, TotalTypes = catalog.Types.Count, Query = query, Results = matches, catalog.Findings }, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else { Console.WriteLine($"DBC\t{catalog.DbcDirectory}\nSHA256\t{catalog.Sha256}\nLiquidTypes\t{catalog.Types.Count:N0}\nMatches\t{matches.Count:N0}"); foreach (var liquid in matches) Console.WriteLine($"LIQUID\t{liquid.Id}\tname={liquid.Name}\tcategory={liquid.Category}\tflags=0x{liquid.Flags:X}\tsound={liquid.SoundId}\tspell={liquid.SpellId}\tmaterial={liquid.MaterialId}\ttextures={string.Join('|', liquid.Textures)}"); foreach (var finding in catalog.Findings) Console.WriteLine($"FINDING\t{finding}"); }
        return catalog.Findings.Count == 0 ? 0 : 3;
    }
    if (args is ["adt-liquid-info", var liquidAdtPath, .. var liquidInfoOptions])
    {
        var json = liquidInfoOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var includeCells = liquidInfoOptions.Contains("--cells", StringComparer.OrdinalIgnoreCase); var dbcRoot = Option(liquidInfoOptions, "--dbc="); var unknown = liquidInfoOptions.Where(option => !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase) && !option.Equals("--cells", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--dbc=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-liquid-info option: {unknown[0]}"); var inspection = AdtLiquidService.Inspect(liquidAdtPath); LiquidTypeCatalog? catalog = string.IsNullOrWhiteSpace(dbcRoot) ? null : LiquidTypeCatalogService.Load(dbcRoot); var decoded = catalog?.Types.Where(value => value.Id <= ushort.MaxValue).ToDictionary(value => (ushort)value.Id);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(new { Inspection = inspection, LiquidTypeCatalog = catalog is null ? null : new { catalog.DbcDirectory, catalog.Sha256, Count = catalog.Types.Count } }, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } }));
        else { Console.WriteLine($"Path\t{inspection.Path}\nSHA256\t{inspection.Sha256}\nMH2OBytes\t{inspection.ChunkPayloadBytes:N0}\nLiquidLayers\t{inspection.Layers.Count:N0}\nCellsWithLiquid\t{inspection.Layers.Select(value => (value.CellX, value.CellY)).Distinct().Count():N0}"); foreach (var group in inspection.Layers.GroupBy(value => value.VertexFormat).OrderBy(value => value.Key)) Console.WriteLine($"FORMAT\t{group.Key}\t{group.Count():N0}"); foreach (var finding in inspection.Findings) Console.WriteLine($"FINDING\t{finding}"); if (includeCells) foreach (var layer in inspection.Layers) { var name = decoded?.GetValueOrDefault(layer.LiquidType)?.Name; Console.WriteLine($"LIQUID\t{layer.CellX},{layer.CellY}\tslot={layer.Slot}\ttype={layer.LiquidType}{(name is null ? string.Empty : $" ({name})")}\tformat={layer.VertexFormat}\trect={layer.OffsetX}:{layer.OffsetY}+{layer.Width}x{layer.Height}\tactive={layer.ActiveTiles}\theights={layer.MinHeight:R}..{layer.MaxHeight:R}\tvertices={layer.Heights.Count}\tactiveVertices={layer.ActiveHeightVertices}\tdepth={layer.DepthValues}\ttexcoords={layer.TextureCoordinatePairs}"); } }
        return inspection.Findings.Count == 0 ? 0 : 3;
    }
    if (args is ["adt-liquid-plan", var planLiquidAdtPath, var liquidLayerText, var liquidTypeText, var liquidModeText, var liquidValueText, var liquidCellText, var liquidPlanPath, .. var liquidPlanOptions])
    {
        var overwrite = liquidPlanOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var dbcRoot = Option(liquidPlanOptions, "--dbc="); var unknown = liquidPlanOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--dbc=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-liquid-plan option: {unknown[0]}"); if (!int.TryParse(liquidLayerText, out var layerSlot) || layerSlot < 0) return Fail("Liquid layer slot must be non-negative."); ushort? targetType = null; if (!liquidTypeText.Equals("keep", StringComparison.OrdinalIgnoreCase)) { if (!ushort.TryParse(liquidTypeText, out var parsedType)) return Fail("Liquid type must be an unsigned 16-bit LiquidType ID or keep."); targetType = parsedType; }
        var normalizedMode = liquidModeText.Replace("-", string.Empty, StringComparison.Ordinal); var heightMode = normalizedMode.Equals("flat", StringComparison.OrdinalIgnoreCase) ? AdtLiquidHeightMode.SetFlat : Enum.TryParse<AdtLiquidHeightMode>(normalizedMode, true, out var parsedMode) && Enum.IsDefined(parsedMode) ? parsedMode : (AdtLiquidHeightMode)(-1); if (!Enum.IsDefined(heightMode)) return Fail("Liquid height mode must be keep, shift, or flat."); if (!float.TryParse(liquidValueText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var heightValue) || !float.IsFinite(heightValue)) return Fail("Liquid height value must be finite."); IReadOnlyList<(int X, int Y)> cells;
        var inspection = AdtLiquidService.Inspect(planLiquidAdtPath); if (liquidCellText.Equals("all", StringComparison.OrdinalIgnoreCase)) cells = inspection.Layers.Where(layer => layer.Slot == layerSlot).Select(layer => (layer.CellX, layer.CellY)).Distinct().ToArray(); else if (!TryParseGridCells(liquidCellText, 16, out cells)) return Fail("ADT cells must use x:y,x:y within the 16x16 grid, or all."); LiquidTypeCatalog? catalog = string.IsNullOrWhiteSpace(dbcRoot) ? null : LiquidTypeCatalogService.Load(dbcRoot); var plan = AdtLiquidService.Plan(planLiquidAdtPath, cells, layerSlot, targetType, heightMode, heightValue, catalog); AdtLiquidService.SavePlan(plan, liquidPlanPath, overwrite); Console.Error.WriteLine($"Planned {plan.Edits.Count:N0} MH2O liquid layer edit(s){(plan.TargetLiquidType is null ? string.Empty : $" to type {plan.TargetLiquidType:N0}")} with {plan.HeightMode} {plan.HeightValue:R}: {Path.GetFullPath(liquidPlanPath)}"); return 0;
    }
    if (args is ["adt-liquid-apply", var applyLiquidPlanPath, var liquidOutputPath, .. var liquidApplyOptions])
    {
        var overwrite = liquidApplyOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = liquidApplyOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-liquid-apply option: {unknown[0]}"); var result = AdtLiquidService.Apply(AdtLiquidService.LoadPlan(applyLiquidPlanPath), liquidOutputPath, overwrite); Console.WriteLine($"Output\t{result.OutputPath}\nSHA256\t{result.OutputSha256}\nReceipt\t{result.ReceiptPath}\nEditedLayers\t{result.EditedLayers:N0}\nEditedCells\t{result.EditedCells:N0}\nEditedVertexHeights\t{result.EditedVertexHeights:N0}"); return 0;
    }
    ...
    if (args is ["adt-ground-effect-plan", var groundAdtPath, var groundLayerText, var groundEffectText, var groundCellText, var groundPlanPath, .. var groundPlanOptions])
    {
        var overwrite = groundPlanOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var dbcRoot = Option(groundPlanOptions, "--dbc="); var unknown = groundPlanOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--dbc=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-ground-effect-plan option: {unknown[0]}"); if (!int.TryParse(groundLayerText, out var layerSlot) || layerSlot < 0 || !int.TryParse(groundEffectText, out var effectId) || effectId < -1) return Fail("Layer slot must be non-negative; effect ID must be -1 (none) or a non-negative GroundEffectTexture ID."); IReadOnlyList<(int X, int Y)> cells;
        var inspection = AdtTextureLayerService.Inspect(groundAdtPath); if (groundCellText.Equals("all", StringComparison.OrdinalIgnoreCase)) cells = inspection.Layers.Where(layer => layer.Slot == layerSlot).Select(layer => (layer.CellX, layer.CellY)).Distinct().ToArray(); else if (!TryParseGridCells(groundCellText, 16, out cells)) return Fail("ADT cells must use x:y,x:y within the 16x16 grid, or all.");
        GroundEffectCatalog? catalog = null; if (!string.IsNullOrWhiteSpace(dbcRoot)) catalog = GroundEffectCatalogService.Load(dbcRoot); var plan = AdtGroundEffectService.Plan(groundAdtPath, cells, layerSlot, effectId, catalog); AdtGroundEffectService.SavePlan(plan, groundPlanPath, overwrite); Console.Error.WriteLine($"Planned {plan.Edits.Count:N0} MCLY ground-effect edit(s) to {plan.EffectId:N0}{(plan.CatalogEvidence is null ? " (raw/unverified ID)" : $" ({plan.CatalogEvidence})")}: {Path.GetFullPath(groundPlanPath)}"); return 0;
    }
    if (args is ["adt-ground-effect-apply", var applyGroundPlanPath, var groundOutputPath, .. var groundApplyOptions])
    {
        var overwrite = groundApplyOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = groundApplyOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-ground-effect-apply option: {unknown[0]}"); var result = AdtGroundEffectService.Apply(AdtGroundEffectService.LoadPlan(applyGroundPlanPath), groundOutputPath, overwrite); Console.WriteLine($"Output\t{result.OutputPath}\nSHA256\t{result.OutputSha256}\nReceipt\t{result.ReceiptPath}\nEditedLayers\t{result.EditedLayers:N0}\nEditedCells\t{result.EditedCells:N0}"); return 0;
    }
    ...
    if (args is ["adt-placement-multi-add-plan", var multiAddAdtPath, var multiAddKindText, var multiAddClientPath, var multiAddAssetPath, var multiAddPlanPath, .. var multiAddOptions])
    {
        var overwrite = multiAddOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = multiAddOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--position=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--orientation=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale-raw=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--uid=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--flags=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--doodad-set=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--name-set=", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-multi-add-plan option: {unknown[0]}");
        if (!Enum.TryParse<AdtPlacementKind>(multiAddKindText, true, out var kind) || !Enum.IsDefined(kind)) return Fail("Placement kind must be m2 or wmo.");
        var positionText = Option(multiAddOptions, "--position="); if (positionText is null || !TryParseVector3(positionText, out var position)) return Fail("--position=x,y,z is required and must contain three finite numbers.");
        var orientation = System.Numerics.Vector3.Zero; var orientationText = Option(multiAddOptions, "--orientation="); if (orientationText is not null && !TryParseVector3(orientationText, out orientation)) return Fail("--orientation must contain three finite comma-separated degree values: x,y,z");
        ushort? scaleRaw = null; var scaleText = Option(multiAddOptions, "--scale="); var scaleRawText = Option(multiAddOptions, "--scale-raw="); if (scaleText is not null && scaleRawText is not null) return Fail("Choose --scale or --scale-raw, not both.");
        if (scaleText is not null) { if (!float.TryParse(scaleText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var scale)) return Fail("--scale must be a finite positive representable number."); scaleRaw = AdtPlacementTransformService.EncodeScale(scale); }
        if (scaleRawText is not null) { if (!TryParseUnsigned(scaleRawText, ushort.MaxValue, out var parsedScale)) return Fail("--scale-raw must be an integer from 0 through 65535; zero is valid only for an effective-scale-1 WMO."); scaleRaw = checked((ushort)parsedScale); }
        uint? uid = null; var uidText = Option(multiAddOptions, "--uid="); if (uidText is not null) { if (!TryParseUnsigned(uidText, uint.MaxValue, out var parsedUid) || parsedUid == 0) return Fail("--uid must be a positive decimal or 0x-prefixed 32-bit integer."); uid = parsedUid; }
        ushort ParseMultiU16(string prefix) { var text = Option(multiAddOptions, prefix); if (text is null) return 0; if (!TryParseUnsigned(text, ushort.MaxValue, out var value)) throw new ArgumentException($"{prefix.TrimEnd('=')} must be a decimal or 0x-prefixed 16-bit integer."); return checked((ushort)value); }
        var plan = AdtMultiTilePlacementService.PlanAdd(multiAddAdtPath, kind, multiAddClientPath, multiAddAssetPath, position, orientation, scaleRaw, uid, ParseMultiU16("--flags="), ParseMultiU16("--doodad-set="), ParseMultiU16("--name-set=")); AdtMultiTilePlacementService.SavePlan(plan, multiAddPlanPath, overwrite);
        Console.WriteLine($"Operation\t{plan.Operation}\nKind\t{plan.Kind}\nUID\t{plan.UniqueId:N0}\nMap\t{plan.MapPrefix}\nTouchedTiles\t{plan.Segments.Count:N0}\nSourceTiles\t{plan.SourceTiles.Count:N0}\nMcrfCells\t{plan.Segments.Sum(segment => segment.ReferencedCells.Count):N0}\nPlan\t{Path.GetFullPath(multiAddPlanPath)}"); foreach (var segment in plan.Segments) Console.WriteLine($"TILE\t{Path.GetFileName(segment.InputPath)}\tindex {segment.Index:N0}\tname {segment.NameId:N0}\tcells {segment.ReferencedCells.Count:N0}"); return 0;
    }
    if (args is ["adt-placement-multi-delete-plan", var multiDeleteAdtPath, var multiDeleteKindText, var multiDeleteIndexText, var multiDeletePlanPath, .. var multiDeleteOptions])
    {
        var overwrite = multiDeleteOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = multiDeleteOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-multi-delete-plan option: {unknown[0]}");
        if (!Enum.TryParse<AdtPlacementKind>(multiDeleteKindText, true, out var kind) || !Enum.IsDefined(kind)) return Fail("Placement kind must be m2 or wmo."); if (!int.TryParse(multiDeleteIndexText, out var index) || index < 0) return Fail("Placement index must be a non-negative integer from asset map-info --placements.");
        var plan = AdtMultiTilePlacementService.PlanDelete(multiDeleteAdtPath, kind, index); AdtMultiTilePlacementService.SavePlan(plan, multiDeletePlanPath, overwrite);
        Console.WriteLine($"Operation\t{plan.Operation}\nKind\t{plan.Kind}\nUID\t{plan.UniqueId:N0}\nMap\t{plan.MapPrefix}\nOccurrenceTiles\t{plan.Segments.Count:N0}\nSourceTiles\t{plan.SourceTiles.Count:N0}\nMcrfCells\t{plan.Segments.Sum(segment => segment.ReferencedCells.Count):N0}\nPlan\t{Path.GetFullPath(multiDeletePlanPath)}"); foreach (var segment in plan.Segments) Console.WriteLine($"TILE\t{Path.GetFileName(segment.InputPath)}\tindex {segment.Index:N0}\tcells {segment.ReferencedCells.Count:N0}"); return 0;
    }
    if (args is ["adt-placement-multi-apply", var multiPlanPath, var multiOutputRoot, .. var multiApplyOptions])
    {
        if (multiApplyOptions.Length > 0) return Fail($"Unknown asset adt-placement-multi-apply option: {multiApplyOptions[0]}");
        var result = AdtMultiTilePlacementService.Apply(AdtMultiTilePlacementService.LoadPlan(multiPlanPath), multiOutputRoot);
        Console.WriteLine($"OutputRoot\t{result.OutputRoot}\nPayload\t{result.PayloadRoot}\nManifest\t{result.ManifestPath}\nReceipt\t{result.ReceiptPath}\nOperation\t{result.Operation}\nKind\t{result.Kind}\nUID\t{result.UniqueId:N0}\nTiles\t{result.Outputs.Count:N0}"); foreach (var output in result.Outputs) Console.WriteLine($"TILE\t{output.TileX},{output.TileY}\t{output.Path}\t{output.Sha256}"); return 0;
    }
    if (args is ["adt-placement-multi-transform-plan", var multiTransformAdtPath, var multiTransformKindText, var multiTransformIndexText, var multiTransformAssetPath, var multiTransformPlanPath, .. var multiTransformOptions])
    {
        var overwrite = multiTransformOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = multiTransformOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--position=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--orientation=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale-raw=", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-multi-transform-plan option: {unknown[0]}");
        if (!Enum.TryParse<AdtPlacementKind>(multiTransformKindText, true, out var kind) || !Enum.IsDefined(kind)) return Fail("Placement kind must be m2 or wmo.");
        if (!int.TryParse(multiTransformIndexText, out var index) || index < 0) return Fail("Placement index must be a non-negative integer from asset map-info --placements.");
        System.Numerics.Vector3? position = null; var positionText = Option(multiTransformOptions, "--position="); if (positionText is not null) { if (!TryParseVector3(positionText, out var parsed)) return Fail("--position must contain three finite comma-separated values: x,y,z"); position = parsed; }
        System.Numerics.Vector3? orientation = null; var orientationText = Option(multiTransformOptions, "--orientation="); if (orientationText is not null) { if (!TryParseVector3(orientationText, out var parsed)) return Fail("--orientation must contain three finite comma-separated degree values: x,y,z"); orientation = parsed; }
        ushort? scaleRaw = null; var scaleText = Option(multiTransformOptions, "--scale="); var scaleRawText = Option(multiTransformOptions, "--scale-raw="); if (scaleText is not null && scaleRawText is not null) return Fail("Choose --scale or --scale-raw, not both.");
        if (scaleText is not null) { if (!float.TryParse(scaleText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var scale)) return Fail("--scale must be a finite positive representable number."); scaleRaw = AdtPlacementTransformService.EncodeScale(scale); }
        if (scaleRawText is not null) { if (!TryParseUnsigned(scaleRawText, ushort.MaxValue, out var parsedScale)) return Fail("--scale-raw must be an integer from 0 through 65535; zero is valid only for an effective-scale-1 WMO."); scaleRaw = checked((ushort)parsedScale); }
        var plan = AdtMultiTilePlacementTransformService.Plan(multiTransformAdtPath, kind, index, multiTransformAssetPath, position, orientation, scaleRaw); AdtMultiTilePlacementTransformService.SavePlan(plan, multiTransformPlanPath, overwrite);
        Console.WriteLine($"Operation\tTransform\nKind\t{plan.Kind}\nUID\t{plan.UniqueId:N0}\nMap\t{plan.MapPrefix}\nOldOccurrenceTiles\t{plan.DeleteSegments.Count:N0}\nEditedFootprintTiles\t{plan.TargetTiles.Count:N0}\nSourceTiles\t{plan.SourceTiles.Count:N0}\nAssetSHA256\t{plan.AssetSha256}\nPlan\t{Path.GetFullPath(multiTransformPlanPath)}"); foreach (var tile in plan.TargetTiles) Console.WriteLine($"TARGET\t{tile.TileX},{tile.TileY}"); return 0;
    }
    if (args is ["adt-placement-multi-transform-apply", var multiTransformApplyPlanPath, var multiTransformOutputRoot, .. var multiTransformApplyOptions])
    {
        if (multiTransformApplyOptions.Length > 0) return Fail($"Unknown asset adt-placement-multi-transform-apply option: {multiTransformApplyOptions[0]}");
        var result = AdtMultiTilePlacementTransformService.Apply(AdtMultiTilePlacementTransformService.LoadPlan(multiTransformApplyPlanPath), multiTransformOutputRoot);
        Console.WriteLine($"OutputRoot\t{result.OutputRoot}\nPayload\t{result.PayloadRoot}\nManifest\t{result.ManifestPath}\nReceipt\t{result.ReceiptPath}\nOperation\tTransform\nKind\t{result.Kind}\nUID\t{result.UniqueId:N0}\nChangedTiles\t{result.Outputs.Count:N0}"); foreach (var output in result.Outputs) Console.WriteLine($"TILE\t{output.TileX},{output.TileY}\t{output.Path}\t{output.Sha256}"); return 0;
    }
    if (args is ["adt-placement-add-plan", var addAdtPath, var addKindText, var addClientPath, var addAssetPath, var addPlanPath, .. var addOptions])
    {
        var overwrite = addOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = addOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--position=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--orientation=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale-raw=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--uid=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--flags=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--doodad-set=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--name-set=", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-add-plan option: {unknown[0]}");
        if (!Enum.TryParse<AdtPlacementKind>(addKindText, true, out var kind) || !Enum.IsDefined(kind)) return Fail("Placement kind must be m2 or wmo.");
        var positionText = Option(addOptions, "--position="); if (positionText is null || !TryParseVector3(positionText, out var position)) return Fail("--position=x,y,z is required and must contain three finite numbers.");
        var orientation = System.Numerics.Vector3.Zero; var orientationText = Option(addOptions, "--orientation="); if (orientationText is not null && !TryParseVector3(orientationText, out orientation)) return Fail("--orientation must contain three finite comma-separated degree values: x,y,z");
        ushort? scaleRaw = null; var scaleText = Option(addOptions, "--scale="); var scaleRawText = Option(addOptions, "--scale-raw="); if (scaleText is not null && scaleRawText is not null) return Fail("Choose --scale or --scale-raw, not both.");
        if (scaleText is not null) { if (!float.TryParse(scaleText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var scale)) return Fail("--scale must be a finite positive representable number."); scaleRaw = AdtPlacementTransformService.EncodeScale(scale); }
        if (scaleRawText is not null) { if (!TryParseUnsigned(scaleRawText, ushort.MaxValue, out var parsedScale)) return Fail("--scale-raw must be an integer from 0 through 65535; zero is valid only for an effective-scale-1 WMO."); scaleRaw = checked((ushort)parsedScale); }
        uint? uid = null; var uidText = Option(addOptions, "--uid="); if (uidText is not null) { if (!TryParseUnsigned(uidText, uint.MaxValue, out var parsedUid) || parsedUid == 0) return Fail("--uid must be a positive decimal or 0x-prefixed 32-bit integer."); uid = parsedUid; }
        ushort ParseU16(string prefix) { var text = Option(addOptions, prefix); if (text is null) return 0; if (!TryParseUnsigned(text, ushort.MaxValue, out var value)) throw new ArgumentException($"{prefix.TrimEnd('=')} must be a decimal or 0x-prefixed 16-bit integer."); return checked((ushort)value); }
        var plan = AdtPlacementLifecycleService.PlanAdd(addAdtPath, kind, addClientPath, addAssetPath, position, orientation, scaleRaw, uid, ParseU16("--flags="), ParseU16("--doodad-set="), ParseU16("--name-set=")); AdtPlacementLifecycleService.SavePlan(plan, addPlanPath, overwrite);
        Console.WriteLine($"Operation\t{plan.Operation}\nKind\t{plan.Kind}\nIndex\t{plan.Index:N0}\nUID\t{plan.UniqueId:N0}\nNameId\t{plan.NameId:N0}\nPath\t{plan.ClientPath}\nPosition\t{plan.Position.X:R},{plan.Position.Y:R},{plan.Position.Z:R}\nOrientation\t{plan.Orientation.X:R},{plan.Orientation.Y:R},{plan.Orientation.Z:R}\nScaleRaw\t{plan.ScaleRaw:N0}\nBounds\t{plan.MinimumExtent!.X:R},{plan.MinimumExtent.Y:R},{plan.MinimumExtent.Z:R}..{plan.MaximumExtent!.X:R},{plan.MaximumExtent.Y:R},{plan.MaximumExtent.Z:R}\nMcrfCells\t{plan.ReferencedCells.Count:N0}\nUidScope\t{plan.OccupancyFilesScanned:N0} sibling tile(s) in {plan.OccupancyDirectory}\nPlan\t{Path.GetFullPath(addPlanPath)}"); return 0;
    }
    if (args is ["adt-placement-delete-plan", var deleteAdtPath, var deleteKindText, var deleteIndexText, var deletePlanPath, .. var deleteOptions])
    {
        var overwrite = deleteOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = deleteOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-delete-plan option: {unknown[0]}");
        if (!Enum.TryParse<AdtPlacementKind>(deleteKindText, true, out var kind) || !Enum.IsDefined(kind)) return Fail("Placement kind must be m2 or wmo."); if (!int.TryParse(deleteIndexText, out var index) || index < 0) return Fail("Placement index must be a non-negative integer from asset map-info --placements.");
        var plan = AdtPlacementLifecycleService.PlanDelete(deleteAdtPath, kind, index); AdtPlacementLifecycleService.SavePlan(plan, deletePlanPath, overwrite); Console.WriteLine($"Operation\t{plan.Operation}\nKind\t{plan.Kind}\nIndex\t{plan.Index:N0}\nUID\t{plan.UniqueId:N0}\nPath\t{plan.ClientPath ?? "<unresolved>"}\nMcrfCells\t{plan.ReferencedCells.Count:N0}\nPlan\t{Path.GetFullPath(deletePlanPath)}"); return 0;
    }
    if (args is ["adt-placement-lifecycle-apply", var lifecyclePlanPath, var lifecycleOutputPath, .. var lifecycleOptions])
    {
        var overwrite = lifecycleOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = lifecycleOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-lifecycle-apply option: {unknown[0]}");
        var result = AdtPlacementLifecycleService.Apply(AdtPlacementLifecycleService.LoadPlan(lifecyclePlanPath), lifecycleOutputPath, overwrite); Console.WriteLine($"Output\t{result.OutputPath}\nSHA256\t{result.OutputSha256}\nReceipt\t{result.ReceiptPath}\nOperation\t{result.Operation}\nKind\t{result.Kind}\nIndex\t{result.Index:N0}\nUID\t{result.UniqueId:N0}\nMcrfCells\t{result.ReferencedCells:N0}"); return 0;
    }
    if (args is ["adt-placement-plan", var placementAdtPath, var placementKindText, var placementIndexText, var placementPlanPath, .. var placementPlanOptions])
    {
        var overwrite = placementPlanOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase);
        var unknown = placementPlanOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--position=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--orientation=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--scale-raw=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--wmo-root=", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-plan option: {unknown[0]}");
        if (!Enum.TryParse<AdtPlacementKind>(placementKindText, true, out var placementKind) || !Enum.IsDefined(placementKind)) return Fail("Placement kind must be m2 or wmo.");
        if (!int.TryParse(placementIndexText, out var placementIndex) || placementIndex < 0) return Fail("Placement index must be a non-negative integer from asset map-info --placements.");
        System.Numerics.Vector3? position = null; var positionText = Option(placementPlanOptions, "--position="); if (positionText is not null) { if (!TryParseVector3(positionText, out var value)) return Fail("--position must contain three finite comma-separated numbers: x,y,z"); position = value; }
        System.Numerics.Vector3? orientation = null; var orientationText = Option(placementPlanOptions, "--orientation="); if (orientationText is not null) { if (!TryParseVector3(orientationText, out var value)) return Fail("--orientation must contain three finite comma-separated degree values: x,y,z"); orientation = value; }
        ushort? scaleRaw = null; var scaleText = Option(placementPlanOptions, "--scale="); var scaleRawText = Option(placementPlanOptions, "--scale-raw=");
        if (scaleText is not null && scaleRawText is not null) return Fail("Choose --scale or --scale-raw, not both.");
        if (scaleText is not null) { if (!float.TryParse(scaleText, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var scale)) return Fail("--scale must be a finite positive number."); scaleRaw = AdtPlacementTransformService.EncodeScale(scale); }
        if (scaleRawText is not null) { if (!ushort.TryParse(scaleRawText, out var raw) || raw == 0) return Fail("--scale-raw must be an integer from 1 through 65535."); scaleRaw = raw; }
        if (position is null && orientation is null && scaleRaw is null) return Fail("Specify at least one changed --position, --orientation, --scale, or --scale-raw value.");
        var wmoRoot = Option(placementPlanOptions, "--wmo-root=");
        if (placementKind == AdtPlacementKind.M2 && wmoRoot is not null) return Fail("--wmo-root applies only to WMO placement rotation/scale plans.");
        var plan = AdtPlacementTransformService.Plan(placementAdtPath, placementKind, placementIndex, position, orientation, scaleRaw, wmoRoot); AdtPlacementTransformService.SavePlan(plan, placementPlanPath, overwrite);
        Console.WriteLine($"Kind\t{plan.Kind}\nIndex\t{plan.Index:N0}\nUID\t{plan.UniqueId:N0}\nPath\t{plan.ClientPath ?? "<unresolved>"}\nPosition\t{plan.OriginalPosition.X:R},{plan.OriginalPosition.Y:R},{plan.OriginalPosition.Z:R} -> {plan.EditedPosition.X:R},{plan.EditedPosition.Y:R},{plan.EditedPosition.Z:R}\nOrientation\t{plan.OriginalOrientation.X:R},{plan.OriginalOrientation.Y:R},{plan.OriginalOrientation.Z:R} -> {plan.EditedOrientation.X:R},{plan.EditedOrientation.Y:R},{plan.EditedOrientation.Z:R}\nScaleRaw\t{plan.OriginalScaleRaw:N0} -> {plan.EditedScaleRaw:N0}\nWmoRoot\t{plan.WmoRootPath ?? "translation-only / not required"}\nExtents\t{(plan.EditedMinimumExtent is null ? "not applicable" : $"{plan.EditedMinimumExtent.X:R},{plan.EditedMinimumExtent.Y:R},{plan.EditedMinimumExtent.Z:R}..{plan.EditedMaximumExtent!.X:R},{plan.EditedMaximumExtent.Y:R},{plan.EditedMaximumExtent.Z:R}")}\nPlan\t{Path.GetFullPath(placementPlanPath)}"); return 0;
    }
    if (args is ["adt-placement-apply", var applyPlacementPlanPath, var placementOutputPath, .. var placementApplyOptions])
    {
        var overwrite = placementApplyOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var unknown = placementApplyOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-placement-apply option: {unknown[0]}");
        var result = AdtPlacementTransformService.Apply(AdtPlacementTransformService.LoadPlan(applyPlacementPlanPath), placementOutputPath, overwrite);
        Console.WriteLine($"Output\t{result.OutputPath}\nSHA256\t{result.OutputSha256}\nReceipt\t{result.ReceiptPath}\nKind\t{result.Kind}\nIndex\t{result.Index:N0}\nUID\t{result.UniqueId:N0}"); return 0;
    }
    if (args is ["map-info", var mapPath, .. var mapOptions])
    {
        var json = mapOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var includeCells = mapOptions.Contains("--cells", StringComparer.OrdinalIgnoreCase); var includePlacements = mapOptions.Contains("--placements", StringComparer.OrdinalIgnoreCase); var includeMaterials = mapOptions.Contains("--materials", StringComparer.OrdinalIgnoreCase); var includeScene = mapOptions.Contains("--scene", StringComparer.OrdinalIgnoreCase) || includeMaterials; var materialLibrary = Option(mapOptions, "--library="); var requestedProvenance = Option(mapOptions, "--provenance=");
        var unknown = mapOptions.Where(option => !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase) && !option.Equals("--cells", StringComparison.OrdinalIgnoreCase) && !option.Equals("--placements", StringComparison.OrdinalIgnoreCase) && !option.Equals("--scene", StringComparison.OrdinalIgnoreCase) && !option.Equals("--materials", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--library=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--provenance=", StringComparison.OrdinalIgnoreCase)).ToArray();
        ...
        if (includeScene && inspection.Kind != MapAssetKind.Adt) return Fail("--scene requires a WotLK ADT because WDT and WDL do not contain MCVT terrain geometry.");
        var scene = includeScene ? AdtTerrainSceneService.Load(mapPath) : null;
        AdtTerrainMaterialSet? materials = null;
        if (includeMaterials)
        {
            if (string.IsNullOrWhiteSpace(materialLibrary) || !Directory.Exists(materialLibrary)) return Fail("--materials requires --library=<processed asset library>.");
            var layout = ClientAssetDependencyService.OpenLibraryLayout(materialLibrary); var layerInspection = AdtTextureLayerService.Inspect(mapPath); var paths = layerInspection.Layers.Select(value => value.TexturePath).Where(value => !string.IsNullOrWhiteSpace(value)).Select(value => value!).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(); var candidates = paths.ToDictionary(path => path, path => ClientAssetDependencyService.FindCandidates(layout, path), StringComparer.OrdinalIgnoreCase);
            var materialProvenance = string.IsNullOrWhiteSpace(requestedProvenance) ? null : requestedProvenance.Trim(); ProcessedAssetEffectiveProfile? effectiveProfile = null;
            if (materialProvenance is null) try { effectiveProfile = ProcessedAssetEffectiveProfileService.Infer(layout, mapPath); materialProvenance = effectiveProfile.DisplayName; } catch (Exception exception) when (exception is InvalidOperationException or InvalidDataException or FileNotFoundException) { }
            if (materialProvenance is null)
            {
                var coherent = candidates.Values.SelectMany(value => value.Select(candidate => candidate.Provenance)).Distinct(StringComparer.OrdinalIgnoreCase).Where(candidate => candidates.Values.All(values => values.Count(value => value.Provenance.Equals(candidate, StringComparison.OrdinalIgnoreCase)) == 1)).ToArray();
                if (coherent.Length == 1) materialProvenance = coherent[0];
                else return Fail($"Terrain material provenance is ambiguous. Pass --provenance=<name>. Candidates: {string.Join(", ", candidates.Values.SelectMany(value => value.Select(candidate => candidate.Provenance)).Distinct(StringComparer.OrdinalIgnoreCase).Order(StringComparer.OrdinalIgnoreCase))}");
            }
            var resolved = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            foreach (var pair in candidates)
            {
                if (effectiveProfile is not null) { var effective = ProcessedAssetEffectiveProfileService.Resolve(layout, effectiveProfile, pair.Key); if (effective.State == ClientEffectiveAssetState.Effective && effective.Effective is not null) resolved[pair.Key] = effective.Effective.SourcePath; }
                else { var exact = pair.Value.Where(candidate => candidate.Provenance.Equals(materialProvenance, StringComparison.OrdinalIgnoreCase)).ToArray(); if (exact.Length == 1) resolved[pair.Key] = exact[0].SourcePath; }
            }
            materials = AdtTerrainMaterialService.Load(mapPath, materialProvenance, resolved);
        }
        var jsonOptions = new System.Text.Json.JsonSerializerOptions { WriteIndented = true, IncludeFields = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } };
        if (json)
        {
            object payload = materials is not null ? new { Inspection = inspection, Scene = scene, Materials = new { materials.AdtPath, materials.Provenance, materials.ResolvedTexturePaths, materials.UnresolvedTexturePaths, materials.CompleteCells, CellCount = materials.Cells.Count, Cells = materials.Cells.Select(cell => new { cell.CellX, cell.CellY, cell.Complete, Layers = cell.Layers }), materials.Findings } } : scene is not null ? new { Inspection = inspection, Scene = scene } : inspection;
            Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(payload, payload.GetType(), jsonOptions));
        }
        else
        {
            ...
            {
                Console.WriteLine($"SCENE\tcells={scene.Cells.Count:N0}\tvertices={scene.Vertices.Count:N0}\ttriangles={scene.TriangleIndices.Count / 3:N0}\tculledHoleSquares={scene.CulledHoleSquares:N0}\tbounds={scene.Minimum.X:R},{scene.Minimum.Y:R},{scene.Minimum.Z:R}..{scene.Maximum.X:R},{scene.Maximum.Y:R},{scene.Maximum.Z:R}");
                foreach (var finding in scene.Findings) Console.WriteLine($"SCENE_FINDING\t{finding}");
            }
            if (materials is not null)
            {
                Console.WriteLine($"MATERIALS\tprovenance={materials.Provenance}\ttextures={materials.ResolvedTexturePaths:N0} resolved/{materials.UnresolvedTexturePaths:N0} unresolved\tcells={materials.CompleteCells:N0}/{materials.Cells.Count:N0} complete\tcomposite={AdtTerrainMaterialService.CompositeSize}x{AdtTerrainMaterialService.CompositeSize}\trepeats={AdtTerrainMaterialService.TextureRepeatsPerCell:N0}");
                foreach (var finding in materials.Findings) Console.WriteLine($"MATERIAL_FINDING\t{finding}");
            }
        ...
        return inspection.Findings.Any(finding => finding.StartsWith("MVER", StringComparison.Ordinal)) || materials is not null && (materials.UnresolvedTexturePaths > 0 || materials.CompleteCells < materials.Cells.Count) ? 3 : 0;
    }
    ...
    if (args is ["wmo-preview-info", var wmoPath, .. var wmoOptions])
    {
        ...
        var geometry = WmoPreviewGeometryService.Load(wmoPath); var rootBounds = WmoPlacementBoundsService.InspectRoot(geometry.RootPath); var textures = WmoPreviewGeometryService.ResolveTextureFiles(geometry, contentRoot);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(new { Geometry = geometry, RootBounds = rootBounds, ResolvedTextureFiles = textures }, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, IncludeFields = true }));
        else
        {
            Console.WriteLine($"Root\t{geometry.RootPath}\nRootSHA256\t{rootBounds.Sha256}\nVersion\t{geometry.Version}\nGroups\t{geometry.Groups.Count:N0}\nVertices\t{geometry.Vertices.Count:N0}\nTriangles\t{geometry.TriangleIndices.Count / 3:N0}\nMaterials\t{geometry.Materials.Count:N0}\nResolvedTextures\t{textures.Count:N0}\nVertexMinimum\t{geometry.Minimum}\nVertexMaximum\t{geometry.Maximum}\nDeclaredMinimum\t{rootBounds.DeclaredMinimum}\nDeclaredMaximum\t{rootBounds.DeclaredMaximum}");
    ...
    if (args is ["character-eye-bind", var eyeInputModel, var eyeInputSkin, var eyeOutputModel, var eyeOutputSkin, .. var eyeOptions])
    {
        ...
        if (json)
            ...
            Console.WriteLine($"CHARACTER_EYE_BIND\tnormal_materials={result.NormalEyeMaterials:N0}\tdk_materials={result.DeathKnightEyeMaterials:N0}\ttextures={result.OriginalTextureDefinitions:N0}->{result.ResultTextureDefinitions:N0}\tlookups={result.OriginalTextureLookups:N0}->{result.ResultTextureLookups:N0}\nMODEL\t{result.OutputModelPath}\nMODEL_SHA256\t{result.OutputModelSha256}\nSKIN\t{result.OutputSkinPath}\nSKIN_SHA256\t{result.OutputSkinSha256}");
        return 0;
    }
...
static async Task<int> Project(string[] args, CancellationToken cancellationToken)
{
    ...
    if (args is ["race-plan", var raceProject, var raceDbc, var raceSchema, var sourceRaceText, var targetRaceText, var targetRaceName, var clientPrefix, var raceFileToken, var raceHost, var racePortText, var raceUser, var raceDatabase, .. var raceOptions])
    {
        if (!uint.TryParse(sourceRaceText, out var sourceRace) || sourceRace is 0 or > 31) return Fail("Source race ID must be from 1 through 31.");
        if (!uint.TryParse(targetRaceText, out var targetRace) || targetRace is 0 or > 31) return Fail("Target race ID must be from 1 through 31.");
        if (!uint.TryParse(racePortText, out var racePort) || racePort is 0 or > 65535) return Fail("Database port must be from 1 to 65535.");
        var output = Option(raceOptions, "--output="); var overwrite = raceOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var json = raceOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var passwordEnvironment = Option(raceOptions, "--password-env=") ?? "WOW_CRUCIBLE_DB_PASSWORD"; var sslText = Option(raceOptions, "--ssl=") ?? "Preferred";
        var allowed = new[] { "--output=", "--password-env=", "--ssl=", "--male-display=", "--female-display=", "--display-source=", "--appearance-race=", "--assets=", "--provenance=" }; var unknown = raceOptions.Where(option => !allowed.Any(prefix => option.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)) && !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown race-plan option: {unknown[0]}");
        uint? maleDisplay = null; var maleText = Option(raceOptions, "--male-display="); if (maleText is not null) { if (!uint.TryParse(maleText, out var parsedMale) || parsedMale == 0) return Fail("--male-display must be a positive CreatureDisplayInfo ID."); maleDisplay = parsedMale; }
        uint? femaleDisplay = null; var femaleText = Option(raceOptions, "--female-display="); if (femaleText is not null) { if (!uint.TryParse(femaleText, out var parsedFemale) || parsedFemale == 0) return Fail("--female-display must be a positive CreatureDisplayInfo ID."); femaleDisplay = parsedFemale; }
        uint? appearanceRace = null; var appearanceRaceText = Option(raceOptions, "--appearance-race="); if (appearanceRaceText is not null) { if (!uint.TryParse(appearanceRaceText, out var parsedAppearanceRace) || parsedAppearanceRace is < 1 or > 31) return Fail("--appearance-race must be from 1 through 31."); appearanceRace = parsedAppearanceRace; }
        var appearance = new PlayableRaceAppearanceOptions(maleDisplay, femaleDisplay, Option(raceOptions, "--assets="), Option(raceOptions, "--provenance="), Option(raceOptions, "--display-source="), appearanceRace);
        var password = Environment.GetEnvironmentVariable(passwordEnvironment); if (password is null) return Fail($"Set the {passwordEnvironment} environment variable for this process. Passwords are not accepted on the command line."); if (!Enum.TryParse<MySqlConnector.MySqlSslMode>(sslText, true, out var ssl)) return Fail($"Unknown SSL mode: {sslText}");
        var profile = new DatabaseConnectionProfile(raceHost, racePort, raceUser, password, raceDatabase, ssl); var capabilities = await new DatabaseCapabilityService().InspectAsync(profile, cancellationToken); var service = new PlayableRaceCloneService(); var plan = await service.CreatePlanAsync(raceProject, raceDbc, raceSchema, sourceRace, targetRace, targetRaceName, clientPrefix, raceFileToken, profile, capabilities, appearance, cancellationToken); if (output is not null) service.SavePlan(output, plan, overwrite);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(plan, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } }));
        else
        {
            Console.WriteLine($"Project\t{plan.ProjectName}\nSourceRace\t{plan.SourceRaceId} · {plan.SourceRaceName}\nTargetRace\t{plan.TargetRaceId} · {plan.TargetRaceName}\nClientPrefix\t{plan.TargetClientPrefix}\nFileToken\t{plan.TargetFileToken}\nDBCChanges\t{plan.DbcRows}\nSQLRows\t{plan.SqlRows}\nAppearancePromotionRows\t{plan.AppearancePromotion?.AddedRows ?? 0}\nCustomizationPromotionRows\t{plan.CustomizationPromotion?.AddedRows ?? 0}\nAppearanceAssets\t{plan.DisplayBindings.SelectMany(binding => binding.Assets).Concat(plan.CustomizationPromotion?.Assets ?? []).Select(asset => asset.ClientPath).Distinct(StringComparer.OrdinalIgnoreCase).Count()}\nReady\t{plan.Ready}\nContentSHA256\t{plan.ContentSha256}");
            if (plan.AppearancePromotion is not null) foreach (var binding in plan.AppearancePromotion.Bindings) Console.WriteLine($"PROMOTE\t{binding.Role}\tDISPLAY={binding.SourceDisplayId}->{binding.TargetDisplayId}\tMODEL={binding.SourceModelId}->{binding.TargetModelId}\tADD={binding.AddedRows}\tREUSE={binding.ReusedRows}");
            if (plan.CustomizationPromotion is not null) foreach (var table in plan.CustomizationPromotion.Tables) Console.WriteLine($"CUSTOMIZE\t{table.Table}\tKEY={table.KeyKind}\tSOURCE={table.SourceRows}\tADD={table.AddedRows}\tREUSE={table.ReusedRows}");
            foreach (var binding in plan.DisplayBindings) Console.WriteLine($"DISPLAY\t{binding.Role}\tID={binding.DisplayId}\tMODEL={binding.ModelId}\tPATH={binding.ModelClientPath}\tPROVENANCE={binding.EffectiveProvenance ?? "target-client"}\tASSETS={binding.Assets.Count}"); foreach (var table in plan.DbcTables) Console.WriteLine($"DBC\t{table.Table}\t{table.AffectedRows}\t{table.Action}"); foreach (var table in plan.SqlTables) Console.WriteLine($"SQL\t{table.Table}\tSOURCE={table.SourceRows}\tPLAN={table.Rows.Count}\tCOVERED={table.AlreadyCovered}\tCONFLICTS={table.Conflicts}"); foreach (var blocker in plan.Blockers) Console.Error.WriteLine($"BLOCKER\t{blocker}"); foreach (var warning in plan.Warnings) Console.Error.WriteLine($"WARNING\t{warning}"); if (output is not null) Console.Error.WriteLine($"Saved reviewable plan: {Path.GetFullPath(output)}");
        }
        return plan.Ready ? 0 : 3;
    }
    if (args is ["race-build", var racePlanPath, var raceOutput, var raceBuildHost, var raceBuildPortText, var raceBuildUser, var raceBuildDatabase, .. var raceBuildOptions])
    {
        if (!uint.TryParse(raceBuildPortText, out var raceBuildPort) || raceBuildPort is 0 or > 65535) return Fail("Database port must be from 1 to 65535."); var apply = raceBuildOptions.Contains("--build", StringComparer.OrdinalIgnoreCase); var passwordEnvironment = Option(raceBuildOptions, "--password-env=") ?? "WOW_CRUCIBLE_DB_PASSWORD"; var sslText = Option(raceBuildOptions, "--ssl=") ?? "Preferred";
        var unknown = raceBuildOptions.Where(option => !option.StartsWith("--password-env=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--ssl=", StringComparison.OrdinalIgnoreCase) && !option.Equals("--build", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown race-build option: {unknown[0]}"); var service = new PlayableRaceCloneService(); var plan = service.LoadPlan(racePlanPath);
        Console.WriteLine($"Plan\t{Path.GetFullPath(racePlanPath)}\nRace\t{plan.SourceRaceId} · {plan.SourceRaceName} -> {plan.TargetRaceId} · {plan.TargetRaceName}\nDBCChanges\t{plan.DbcRows}\nSQLRows\t{plan.SqlRows}\nReady\t{plan.Ready}\nContentSHA256\t{plan.ContentSha256}"); if (!apply) { Console.Error.WriteLine("Dry run only. Re-run with --build to revalidate live inputs and create the new bundle."); return plan.Ready ? 0 : 3; }
        var password = Environment.GetEnvironmentVariable(passwordEnvironment); if (password is null) return Fail($"Set the {passwordEnvironment} environment variable for this process. Passwords are not accepted on the command line."); if (!Enum.TryParse<MySqlConnector.MySqlSslMode>(sslText, true, out var ssl)) return Fail($"Unknown SSL mode: {sslText}"); var result = await service.BuildAsync(plan, new DatabaseConnectionProfile(raceBuildHost, raceBuildPort, raceBuildUser, password, raceBuildDatabase, ssl), raceOutput, cancellationToken); Console.Error.WriteLine($"Built playable-race bundle: {result.OutputRoot}\nPatch: {result.PatchPath}\nSQL: {result.SqlPath}\nManifest: {result.ManifestPath}\nReceipt: {result.ReceiptPath}"); return 0;
    }
...
static int Client(string[] args, CancellationToken cancellationToken)
{
    if (args.Length == 0 || args[0] is "help" or "--help" or "-h") return ClientHelp();
    if (args is ["addon-audit", var addonRoot, .. var addonOptions])
    {
        var unknown = addonOptions.FirstOrDefault(option => !option.StartsWith("--exclude=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--interface=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--output=", StringComparison.OrdinalIgnoreCase));
        if (unknown is not null) return Fail($"Unknown addon audit option: {unknown}");
        var target = Option(addonOptions, "--interface=") is { } value ? int.Parse(value, CultureInfo.InvariantCulture) : 30300;
        var report = AddonAuditService.Scan(addonRoot, target, addonOptions.Where(option => option.StartsWith("--exclude=", StringComparison.OrdinalIgnoreCase)).Select(option => option[10..]).ToArray(), cancellationToken);
        var json = System.Text.Json.JsonSerializer.Serialize(report, new System.Text.Json.JsonSerializerOptions { WriteIndented = true });
        if (Option(addonOptions, "--output=") is { } output) File.WriteAllText(output, json); else Console.WriteLine(json);
        Console.Error.WriteLine($"Addon audit: {report.Packages.Count} packages, {report.Packages.Sum(package => package.Errors)} load errors, {report.DiscoveryIssues.Count} discovery notices. Static checks only; in-game behavior is not verified.");
        return report.DiscoveryIssues.Any(issue => issue.Severity == "Error") || report.Packages.Any(package => package.Errors > 0) ? 3 : 0;
    }
...
static async Task<int> Database(string[] args, CancellationToken cancellationToken)
{
    ...
    if (args[0].Equals("sync-inspect", StringComparison.OrdinalIgnoreCase))
    {
        ...
        foreach (var lookup in plan.LookupEvidence ?? []) Console.WriteLine($"LOOKUP\t{lookup.Source}\t{lookup.SourceDisplayIdentity}\t{lookup.LookupName}\t{lookup.LookupTable}\t{string.Join(',', lookup.Match.Select(part => $"{part.Column}={part.Value.State}:{part.Value.Value}"))}\t{lookup.ResultColumn}={lookup.Result.State}:{lookup.Result.Value}\t{lookup.MatchedIdentity}");
    ...
    if (args[0].Equals("sync-bridge-inspect", StringComparison.OrdinalIgnoreCase))
    {
        ...
        foreach (var table in bridgeProfile.Tables)
        {
            ...
            Console.WriteLine($"TABLE\t{table.SourceTable}\t{tableTarget}\tobserved={table.ObservedSourceColumns?.Count ?? 0}\tkeys={string.Join(',', table.SourcePrimaryKeyColumns ?? [])}\tinsert-defaults={table.RequiresInsertDefaults}\tsuppress-primary={table.SuppressPrimaryOutput}\texpansions={table.Expansions?.Count ?? 0}\tlookups={table.Lookups?.Count ?? 0}");
            ...
            {
                var blocked = string.IsNullOrWhiteSpace(lookup.Table) || string.IsNullOrWhiteSpace(lookup.ResultColumn) || lookup.Matches.Count == 0; if (blocked) unresolved++;
                Console.WriteLine($"LOOKUP\t{table.SourceTable}/{lookup.Name}\t{lookup.Source}\t{(string.IsNullOrWhiteSpace(lookup.Table) ? "UNRESOLVED" : lookup.Table)}\tresult={lookup.ResultColumn}\tversion={lookup.ResultVersion}\tmatches={lookup.Matches.Count}");
                foreach (var match in lookup.Matches) { if (string.IsNullOrWhiteSpace(match.LookupColumn) || UnresolvedExpansionValue(match.Input, false)) unresolved++; Console.WriteLine($"LOOKUP_MATCH\t{lookup.Table}.{match.LookupColumn}\t{DescribeExpansionValue(match.Input)}"); }
            }
            foreach (var expansion in table.Expansions ?? [])
            {
                ...
                foreach (var key in expansion.KeyBindings) { if (UnresolvedExpansionValue(key.Value, true)) unresolved++; Console.WriteLine($"EXPANSION_KEY\t{expansion.TargetTable}.{key.TargetColumn}\t{DescribeExpansionValue(key.Value)}"); }
                foreach (var field in expansion.FieldBindings)
                {
                    ...
                    if ((beforeRequired && (field.Before is null || UnresolvedExpansionValue(field.Before, true))) || (afterRequired && (field.After is null || UnresolvedExpansionValue(field.After, true)))) unresolved++;
        ...
        static bool UnresolvedExpansionValue(DatabaseSyncExpansionValue value, bool allowLookup) => value.Source switch
        {
        ...
        static string DescribeExpansionValue(DatabaseSyncExpansionValue? value)
        {
            if (value is null) return "UNRESOLVED";
            var source = value.Source switch { DatabaseSyncExpansionValueSource.Constant => $"Constant:{value.Constant?.State}:{value.Constant?.Value}", DatabaseSyncExpansionValueSource.Lookup => $"Lookup:{value.LookupName}", _ => $"{value.Source}:{value.SourceColumn}" };
            if (value.Transform is null or { Kind: DatabaseSyncValueTransformKind.None }) return source;
            var detail = value.Transform.Kind switch { DatabaseSyncValueTransformKind.ExactMap => $"{value.Transform.Mappings?.Count ?? 0} typed mapping(s)", DatabaseSyncValueTransformKind.NullFallback => $"{value.Transform.Fallback?.State}:{value.Transform.Fallback?.Value}", _ => value.Transform.Operand };
            return $"{source} | {value.Transform.Kind}:{detail}";
        }
    }
    ...
    if (args[0].Equals("object-compose", StringComparison.OrdinalIgnoreCase))
    {
        if (args.Length < 5 || args[4].StartsWith("--", StringComparison.Ordinal)) return Fail("db object-compose requires <database> <trigger|procedure|function|event> <name> <body.sql>.");
        var composeDatabase = args[1]; if (!Enum.TryParse<SqlDatabaseObjectType>(args[2], true, out var type) || type == SqlDatabaseObjectType.View) return Fail("Guided object composition supports trigger, procedure, function, or event. Use view-set for guided views.");
        var name = args[3]; var bodyPath = Path.GetFullPath(args[4]); if (!File.Exists(bodyPath)) return Fail($"Guided object body file not found: {bodyPath}"); var options = args[5..];
        var unknown = options.Where(option =>
            !option.StartsWith("--table=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--timing=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--event=", StringComparison.OrdinalIgnoreCase) &&
            !option.StartsWith("--parameters=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--returns=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--data-access=", StringComparison.OrdinalIgnoreCase) &&
            !option.StartsWith("--security=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--every=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--unit=", StringComparison.OrdinalIgnoreCase) &&
            !option.StartsWith("--at=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--starts=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--ends=", StringComparison.OrdinalIgnoreCase) &&
            !option.StartsWith("--output=", StringComparison.OrdinalIgnoreCase) && !option.Equals("--deterministic", StringComparison.OrdinalIgnoreCase) && !option.Equals("--not-deterministic", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--preserve", StringComparison.OrdinalIgnoreCase) && !option.Equals("--disabled", StringComparison.OrdinalIgnoreCase) && !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown object-compose option: {unknown[0]}");
        var body = await File.ReadAllTextAsync(bodyPath, cancellationToken); var securityText = Option(options, "--security=") ?? "invoker";
        if (!Enum.TryParse<SqlSecurityMode>(securityText, true, out var security)) return Fail("--security must be invoker or definer.");
        var accessText = (Option(options, "--data-access=") ?? "contains-sql").Replace("-", string.Empty, StringComparison.Ordinal);
        if (!Enum.TryParse<SqlRoutineDataAccess>(accessText, true, out var access)) return Fail("--data-access must be contains-sql, no-sql, reads-sql-data, or modifies-sql-data.");
        var parameterPath = Option(options, "--parameters="); var parameterText = parameterPath is null ? string.Empty : File.Exists(parameterPath) ? await File.ReadAllTextAsync(Path.GetFullPath(parameterPath), cancellationToken) : throw new FileNotFoundException("Routine parameter file not found.", parameterPath);
        string definition;
        switch (type)
        {
            case SqlDatabaseObjectType.Trigger:
                var table = Option(options, "--table=") ?? throw new ArgumentException("Guided trigger composition requires --table=name.");
                if (!Enum.TryParse<SqlTriggerTiming>(Option(options, "--timing=") ?? "before", true, out var timing)) return Fail("--timing must be before or after.");
                if (!Enum.TryParse<SqlTriggerEvent>(Option(options, "--event=") ?? "update", true, out var triggerEvent)) return Fail("--event must be insert, update, or delete.");
                definition = SqlDatabaseObjectComposer.BuildTrigger(composeDatabase, name, table, timing, triggerEvent, body); break;
            case SqlDatabaseObjectType.Procedure:
                definition = SqlDatabaseObjectComposer.BuildProcedure(composeDatabase, name, SqlDatabaseObjectComposer.ParseParameters(type, parameterText), access, security, body); break;
            case SqlDatabaseObjectType.Function:
                if (options.Contains("--deterministic", StringComparer.OrdinalIgnoreCase) && options.Contains("--not-deterministic", StringComparer.OrdinalIgnoreCase)) return Fail("Choose either --deterministic or --not-deterministic, not both.");
                var returns = Option(options, "--returns=") ?? throw new ArgumentException("Guided function composition requires --returns=TYPE.");
                definition = SqlDatabaseObjectComposer.BuildFunction(composeDatabase, name, SqlDatabaseObjectComposer.ParseParameters(type, parameterText), returns, !options.Contains("--not-deterministic", StringComparer.OrdinalIgnoreCase), access, security, body); break;
            case SqlDatabaseObjectType.Event:
                var everyText = Option(options, "--every="); var at = Option(options, "--at="); if ((everyText is null) == (at is null)) return Fail("Guided event composition requires exactly one schedule: --every=N with --unit=UNIT, or --at=\"yyyy-MM-dd HH:mm:ss\".");
                if (everyText is not null)
                {
                    if (!int.TryParse(everyText, out var every) || every < 1) return Fail("--every must be a positive integer.");
                    if (!Enum.TryParse<SqlEventIntervalUnit>(Option(options, "--unit=") ?? string.Empty, true, out var unit)) return Fail("Recurring events require --unit=second|minute|hour|day|week|month|quarter|year.");
                    definition = SqlDatabaseObjectComposer.BuildRecurringEvent(composeDatabase, name, every, unit, Option(options, "--starts="), Option(options, "--ends="), options.Contains("--preserve", StringComparer.OrdinalIgnoreCase), !options.Contains("--disabled", StringComparer.OrdinalIgnoreCase), body);
                }
                else definition = SqlDatabaseObjectComposer.BuildOneTimeEvent(composeDatabase, name, at!, options.Contains("--preserve", StringComparer.OrdinalIgnoreCase), !options.Contains("--disabled", StringComparer.OrdinalIgnoreCase), body);
                break;
            default: throw new ArgumentOutOfRangeException();
        }
        var output = Option(options, "--output=");
        if (output is null) Console.WriteLine(definition); else { output = Path.GetFullPath(output); if (File.Exists(output) && !options.Contains("--overwrite", StringComparer.OrdinalIgnoreCase)) return Fail($"Output already exists: {output}. Use --overwrite intentionally."); WriteTextAtomic(output, definition + Environment.NewLine); Console.Error.WriteLine($"Exact guided definition: {output}"); }
        return 0;
    }
    ...
    if (operation.Equals("object-dependencies", StringComparison.OrdinalIgnoreCase))
    {
        if (args.Length < 7 || args[5].StartsWith("--", StringComparison.Ordinal) || args[6].StartsWith("--", StringComparison.Ordinal)) return Fail("db object-dependencies requires <view|trigger|procedure|function|event> <name>.");
        var options = args[7..]; var draftPath = Option(options, "--draft="); var json = options.Any(option => option.Equals("--format=json", StringComparison.OrdinalIgnoreCase));
        var unknown = options.Where(option => !option.StartsWith("--password-env=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--ssl=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--draft=", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown object-dependencies option: {unknown[0]}"); if (draftPath is not null && !File.Exists(draftPath)) return Fail($"Draft CREATE definition not found: {draftPath}");
        var type = ParseDatabaseObjectType(args[5]); var name = args[6]; string definition;
        if (draftPath is not null) definition = await File.ReadAllTextAsync(draftPath, cancellationToken);
        else
        {
            var service = new SqlDatabaseObjectService(); var item = (await service.ListAsync(profile, cancellationToken)).FirstOrDefault(candidate => candidate.Type == type && candidate.Name.Equals(name, StringComparison.OrdinalIgnoreCase));
            if (item is null) return Fail($"{type} not found: {name}. Supply --draft=<create.sql> to analyze a new identity."); definition = (await service.ShowCreateAsync(profile, item, cancellationToken)).CreateSql;
        }
        var report = await new SqlDatabaseObjectDependencyService().AnalyzeAsync(profile, type, name, definition, cancellationToken);
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(report, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } })); else PrintObjectDependencyImpact(report);
        return 0;
    }
    ...
    if (operation.Equals("object-set", StringComparison.OrdinalIgnoreCase))
    {
        if (args.Length < 8 || args[5].StartsWith("--", StringComparison.Ordinal) || args[6].StartsWith("--", StringComparison.Ordinal) || args[7].StartsWith("--", StringComparison.Ordinal)) return Fail("db object-set requires <view|trigger|procedure|function|event> <name> <create-definition.sql>.");
        var options = args[8..]; var apply = options.Any(option => option.Equals("--apply", StringComparison.OrdinalIgnoreCase)); var json = options.Any(option => option.Equals("--format=json", StringComparison.OrdinalIgnoreCase));
        var unknown = options.Where(option => !option.StartsWith("--password-env=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--ssl=", StringComparison.OrdinalIgnoreCase) && !option.Equals("--apply", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown object-set option: {unknown[0]}"); if (!File.Exists(args[7])) return Fail($"CREATE definition file not found: {args[7]}");
        var type = ParseDatabaseObjectType(args[5]); var definition = await File.ReadAllTextAsync(args[7], cancellationToken); var service = new SqlDatabaseObjectService(); var plan = await service.PrepareChangeAsync(profile, type, args[6], definition, cancellationToken);
        if (!apply)
        {
            if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(plan, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } }));
            else { Console.WriteLine(plan.ReviewSql); foreach (var warning in plan.Warnings) Console.Error.WriteLine($"WARNING: {warning}"); }
            Console.Error.WriteLine($"Dry-run {(plan.ReplacesExisting ? "replacement" : "creation")} only. Re-run with --apply after reviewing the exact definition and target identity."); return 0;
        }
        var result = await service.ApplyChangeAsync(profile, plan, cancellationToken); if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(result, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } })); else Console.WriteLine($"APPLIED\t{result.Receipt.Type}\t{result.Receipt.Database}.{result.Receipt.Name}\nSTATE\t{result.Receipt.State}\nBEFORE_SHA256\t{result.Receipt.BeforeCreateSqlSha256 ?? "-"}\nAFTER_SHA256\t{result.Receipt.AfterCreateSqlSha256}\nRECEIPT\t{result.ReceiptPath}"); return 0;
    }
    if (operation.Equals("object-rollback", StringComparison.OrdinalIgnoreCase))
    {
        if (args.Length < 6 || args[5].StartsWith("--", StringComparison.Ordinal)) return Fail("db object-rollback requires <receipt.json>."); var options = args[6..]; var apply = options.Any(option => option.Equals("--apply", StringComparison.OrdinalIgnoreCase)); var json = options.Any(option => option.Equals("--format=json", StringComparison.OrdinalIgnoreCase));
        var unknown = options.Where(option => !option.StartsWith("--password-env=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--ssl=", StringComparison.OrdinalIgnoreCase) && !option.Equals("--apply", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown object-rollback option: {unknown[0]}"); if (!File.Exists(args[5])) return Fail($"SQL object receipt not found: {args[5]}");
        var service = new SqlDatabaseObjectService(); var receipt = await service.LoadReceiptAsync(args[5], cancellationToken); if (!apply) { if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(receipt, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } })); else Console.WriteLine($"ROLLBACK\t{receipt.Type}\t{receipt.Database}.{receipt.Name}\t{(receipt.BeforeCreateSql is null ? "remove newly created object" : "restore exact pre-change definition")}\nCURRENT_EXPECTED_SHA256\t{receipt.AfterCreateSqlSha256}\nRECEIPT\t{Path.GetFullPath(args[5])}"); Console.Error.WriteLine("Dry-run rollback only. Re-run with --apply; any changed live postimage is refused."); return 0; }
        var result = await service.RollbackChangeAsync(profile, args[5], cancellationToken); if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(result, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } })); else Console.WriteLine($"ROLLED_BACK\t{result.Receipt.Type}\t{result.Receipt.Database}.{result.Receipt.Name}\nRECEIPT\t{result.ReceiptPath}"); return 0;
    }
...
static int Manifest(string[] args)
{
    ...
    if (args is ["create", var manifestPath, var outputFile, .. var rawInputs] && rawInputs.Length > 0)
    {
        ...
        var locale = MpqLocale.Parse(localeOption?[9..]); var entries = PatchInputMapper.Map(inputs).Select(entry => entry with { Locale = locale }).ToArray();
    ...
    switch (args)
    {
        ...
        case ["list", var listManifestPath]:
            ...
            foreach (var entry in listManifest.Entries.OrderBy(entry => entry.ArchivePath, StringComparer.OrdinalIgnoreCase).ThenBy(entry => entry.Locale)) Console.WriteLine($"{entry.SourcePath}\t{entry.ArchivePath}\t{MpqLocale.Format(entry.Locale)}");
...
static int Dbc(string[] args)
{
    ...
    if (args is ["charsections-hd-promote", var primaryCharSections, var hdCharSections, var promotedCharSections, .. var promotionOptions])
    {
        var overwrite = promotionOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase);
        var json = promotionOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase);
        var unknown = promotionOptions.Where(option =>
            !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown charsections-hd-promote option: {unknown[0]}");
        var result = CharSectionsHdCompatibilityService.Promote(primaryCharSections, hdCharSections, promotedCharSections, overwrite);
        if (json)
            Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(result, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else
            Console.WriteLine($"CHARSECTIONS_HD_PROMOTE\tprimary={result.PrimaryRows:N0}\thd={result.CompanionRows:N0}\tby_id={result.MatchedById:N0}\tby_selector={result.MatchedBySelector:N0}\tappended={result.AppendedRows:N0}\tchanged_rows={result.ChangedRows:N0}\tchanged_cells={result.ChangedCells:N0}\tresult={result.ResultRows:N0}\nOUTPUT\t{result.OutputPath}\nSHA256\t{result.OutputSha256}");
        return 0;
    }
    if (args is ["charsections-expose-dk-palettes", var paletteInput, var paletteOutput, .. var paletteOptions])
    {
        var raceText = Option(paletteOptions, "--races=");
        var overwrite = paletteOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase);
        var json = paletteOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase);
        var unknown = paletteOptions.Where(option =>
            !option.StartsWith("--races=", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown charsections-expose-dk-palettes option: {unknown[0]}");
        if (string.IsNullOrWhiteSpace(raceText)) return Fail("charsections-expose-dk-palettes requires --races=4,11.");
        var raceIds = raceText.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Select(value => uint.TryParse(value, out var parsed) && parsed > 0
                ? parsed
                : throw new ArgumentException($"Invalid positive race ID: {value}"))
            .ToArray();
        var result = CharSectionsDeathKnightPaletteCompatibilityService.Expose(
            paletteInput,
            paletteOutput,
            raceIds,
            overwrite);
        if (json)
            ...
            Console.WriteLine($"CHARSECTIONS_EXPOSE_DK_PALETTES\traces={string.Join(',', result.RaceIds)}\tsurfaces={result.DeathKnightPaletteSurfaces:N0}\tcandidates={result.CandidateRows:N0}\texisting={result.ExistingNormalRows:N0}\tappended={result.AppendedRows:N0}\trows={result.InputRows:N0}->{result.ResultRows:N0}\nOUTPUT\t{result.OutputPath}\nSHA256\t{result.OutputSha256}");
        return 0;
    }
    if (args is ["hd-facial-styles-promote", var ordinaryFacialStyles, var hdFacialStyles, var promotedFacialStyles, .. var facialStyleOptions])
    {
        var overwrite = facialStyleOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase);
        var json = facialStyleOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase);
        var unknown = facialStyleOptions.Where(option =>
            !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) &&
            !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray();
        if (unknown.Length > 0) return Fail($"Unknown hd-facial-styles-promote option: {unknown[0]}");
        var result = HdFacialStyleCompatibilityService.Promote(ordinaryFacialStyles, hdFacialStyles, promotedFacialStyles, overwrite);
        if (json)
            Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(result, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        else
            Console.WriteLine($"HD_FACIAL_STYLES_PROMOTE\tordinary={result.OrdinaryRows:N0}\thd={result.HdRows:N0}\tcompatible_surfaces={result.CompatibleSurfaces:N0}\tincompatible_surfaces={result.IncompatibleSurfaces:N0}\tappended={result.AppendedRows:N0}\tskipped={result.SkippedMissingRows:N0}\tresult={result.ResultRows:N0}\nOUTPUT\t{result.OutputPath}\nSHA256\t{result.OutputSha256}");
        return 0;
    }
...
static int Mpq(string[] args)
{
    ...
    switch (args[0])
    {
        case "list" when args.Length >= 2:
            {
                ...
                else foreach (var file in files) Console.WriteLine($"{file.Size}\t{file.CompressedSize}\t{MpqLocale.Format(file.Locale)}\t{file.ArchivePath}");
        ...
        case "extract-folder" when args.Length >= 4:
            {
                var options = args[4..]; var quiet = options.Any(option => option.Equals("--quiet", StringComparison.OrdinalIgnoreCase)); var listFile = Option(options, "--listfile="); var progressOption = options.FirstOrDefault(option => option.StartsWith("--progress=", StringComparison.OrdinalIgnoreCase)); var progressStep = progressOption is null ? 5 : int.Parse(progressOption[11..]); var workerText = Option(options, "--workers="); var workers = workerText is null ? 0 : int.Parse(workerText); var failureReport = Option(options, "--continue-on-error=");
                if (progressStep is < 1 or > 100) throw new ArgumentOutOfRangeException(nameof(progressStep), "Progress percentage must be from 1 to 100."); if (workerText is not null && workers is (< 1 or > PatchArchiveService.MaximumExtractionWorkers)) throw new ArgumentOutOfRangeException(nameof(workers), $"Extraction workers must be from 1 to {PatchArchiveService.MaximumExtractionWorkers}."); if (failureReport is not null && string.IsNullOrWhiteSpace(failureReport)) return Fail("--continue-on-error requires a non-empty CSV report path."); var unknown = options.Where(option => option.StartsWith("--", StringComparison.Ordinal) && !option.Equals("--quiet", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--progress=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--listfile=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--workers=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--continue-on-error=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown extract-folder option: {unknown[0]}");
                var allFiles = LoadMpqIndex(service, args[1], listFile); var files = MpqArchiveBrowser.SelectFolder(allFiles, args[2]); if (files.Count == 0) return Fail($"MPQ folder not found or empty: {args[2]}"); PrintAnonymousMpqWarning(allFiles, listFile); var failures = failureReport is null ? null : new List<(MpqFileEntry Entry, Exception Error)>(); var timer = Stopwatch.StartNew(); service.Extract(args[1], args[3], files, quiet ? null : new ConsoleProgress(progressStep), extractionFailure: failures is null ? null : (entry, error) => failures.Add((entry, error)), workers: workers); var failed = failures?.Count ?? 0; if (failureReport is not null) WriteMpqExtractionFailureReport(args[1], failureReport, failures!); Console.Error.WriteLine($"Extracted {files.Count - failed:N0} of {files.Count:N0} recursive file(s) from {args[2]} to {Path.GetFullPath(args[3])} in {timer.Elapsed.TotalSeconds:0.##}s using {(workers == 0 ? $"auto (up to {PatchArchiveService.RecommendedExtractionWorkers})" : workers.ToString())} worker(s).{(failureReport is null ? string.Empty : $" Failed: {failed:N0}. Report: {Path.GetFullPath(failureReport)}")}"); return failed == 0 ? 0 : 4;
            }
        case "extract" when args.Length >= 3:
            {
                var options = args[3..];
                ...
                var listFile = Option(options, "--listfile=");
                var failureReport = Option(options, "--continue-on-error=");
                ...
                if (workerText is not null && workers is (< 1 or > PatchArchiveService.MaximumExtractionWorkers)) throw new ArgumentOutOfRangeException(nameof(workers), $"Extraction workers must be from 1 to {PatchArchiveService.MaximumExtractionWorkers}.");
                if (failureReport is not null && string.IsNullOrWhiteSpace(failureReport)) return Fail("--continue-on-error requires a non-empty CSV report path.");
                var unknown = options.Where(option => option.StartsWith("--", StringComparison.Ordinal) && !option.Equals("--quiet", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--progress=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--listfile=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--workers=", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--continue-on-error=", StringComparison.OrdinalIgnoreCase)).ToArray();
                ...
                var failures = failureReport is null ? null : new List<(MpqFileEntry Entry, Exception Error)>();
                service.Extract(args[1], args[2], files, quiet ? null : new ConsoleProgress(progressStep), extractionFailure: failures is null ? null : (entry, error) => failures.Add((entry, error)), workers: workers);
                var failed = failures?.Count ?? 0;
                if (failureReport is not null) WriteMpqExtractionFailureReport(args[1], failureReport, failures!);
                Console.Error.WriteLine($"Extracted {files.Length - failed:N0} of {files.Length:N0} file(s) to {Path.GetFullPath(args[2])} in {timer.Elapsed.TotalSeconds:0.##}s using {(workers == 0 ? $"auto (up to {PatchArchiveService.RecommendedExtractionWorkers})" : workers.ToString())} worker(s).{(failureReport is null ? string.Empty : $" Failed: {failed:N0}. Report: {Path.GetFullPath(failureReport)}")}");
                return failed == 0 ? 0 : 4;
            }
        case "merge" when args.Length >= 4:
            {
                ...
                foreach (var conflict in result.Conflicts) Console.WriteLine($"CONFLICT\t{conflict.ArchivePath}\t{MpqLocale.Format(conflict.Locale)}\t{string.Join('|', conflict.Sources)}\t{string.Join('|', conflict.Sha256)}");
                if (result.OutputFiles == 0 && result.Conflicts.Count > 0) { Console.Error.WriteLine($"Merge blocked by {result.Conflicts.Count:N0} different-byte path+locale conflict(s); source archives and output were not modified."); return 3; }
                Console.Error.WriteLine($"Merged {result.InputArchives:N0} source patches into {result.OutputPath}: {result.OutputFiles:N0} physical entries, {result.LocaleVariantEntries:N0} same-path locale entries, {result.ExactDuplicates:N0} exact same-identity duplicate(s), {result.Conflicts.Count:N0} explicitly resolved conflict(s) using {result.Policy}."); return 0;
            }
...
static void WriteMpqExtractionFailureReport(string archivePath, string reportPath, IReadOnlyList<(MpqFileEntry Entry, Exception Error)> failures)
{
    ...
    var report = new System.Text.StringBuilder("Archive,ArchivePath,Locale,BlockIndex,Size,CompressedSize,Flags,ErrorType,Error\n");
    foreach (var failure in failures.OrderBy(item => item.Entry.ArchivePath, StringComparer.OrdinalIgnoreCase).ThenBy(item => item.Entry.Locale).ThenBy(item => item.Entry.BlockIndex))
    {
        report.AppendLine(string.Join(',',
            Csv(Path.GetFullPath(archivePath)),
            Csv(failure.Entry.ArchivePath),
            Csv(MpqLocale.Format(failure.Entry.Locale)),
            Csv(failure.Entry.BlockIndex),
            Csv(failure.Entry.Size),
            Csv(failure.Entry.CompressedSize),
            Csv($"0x{failure.Entry.Flags:X8}"),
            Csv(failure.Error.GetType().FullName),
            Csv(failure.Error.Message)));
    }
    WriteTextAtomic(reportPath, report.ToString());
}
...
static int GroupHelp(string message, int code)
{
    ...
    if (message.Contains("wowcrucible asset texture-info", StringComparison.Ordinal))
        message = message.Replace("Usage:\n", "Usage:\n  wowcrucible asset map-info <file.adt|wdt|wdl> [--cells] [--placements] [--scene] [--materials --library=processed-assets [--provenance=name]] [--format=text|json]\n  wowcrucible asset wdt-create <output.wdt> [--tiles=x:y,x:y] [--format=text|json] [--overwrite]\n  wowcrucible asset wdt-tiles-plan <input.wdt> <present|absent> <x:y,x:y|all|all-present|all-missing> <plan.json> [--format=text|json] [--overwrite]\n  wowcrucible asset wdt-tiles-apply <plan.json> <output.wdt> [--format=text|json] [--overwrite]\n  wowcrucible asset adt-height-plan <input.adt> <delta> <x:y,x:y|all> <plan.json> [--overwrite]\n  wowcrucible asset adt-height-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset adt-brush-plan <input.adt> <center-x:center-y> <radius> <strength> <plan.json> [--mode=raise-lower|flatten|smooth|noise] [--target-height=N] [--seed=N] [--falloff=linear|smooth|constant] [--overwrite]\n  wowcrucible asset adt-brush-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset liquid-type-catalog <dbc-folder> [--query=terms] [--limit=N] [--format=text|json]\n  wowcrucible asset adt-liquid-info <input.adt> [--cells] [--dbc=folder] [--format=text|json]\n  wowcrucible asset adt-liquid-plan <input.adt> <layer-slot> <type-id|keep> <keep|shift|flat> <height-value> <x:y,x:y|all> <plan.json> [--dbc=folder] [--overwrite]\n  wowcrucible asset adt-liquid-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset adt-texture-info <input.adt> [--cells] [--format=text|json]\n  wowcrucible asset adt-texture-plan <input.adt> <layer-slot> <texture-id> <x:y,x:y|all> <plan.json> [--overwrite]\n  wowcrucible asset adt-texture-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset ground-effect-catalog <dbc-folder> [--query=terms] [--limit=N] [--format=text|json]\n  wowcrucible asset adt-ground-effect-plan <input.adt> <layer-slot> <effect-id|-1> <x:y,x:y|all> <plan.json> [--dbc=folder] [--overwrite]\n  wowcrucible asset adt-ground-effect-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset adt-texture-add-plan <input.adt> <client-texture.blp> <x:y,x:y> <plan.json> [--encoding=auto|packed-4-bit|big-8-bit|rle-8-bit] [--initial-alpha=0] [--overwrite]\n  wowcrucible asset adt-texture-add-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset adt-alpha-info <input.adt> [--cells] [--format=text|json]\n  wowcrucible asset adt-alpha-plan <input.adt> <layer-slot> <center-x:center-y> <radius> <target-alpha> <opacity> <x:y,x:y|all> <plan.json> [--falloff=linear|smooth|constant] [--overwrite]\n  wowcrucible asset adt-alpha-apply <plan.json> <output.adt> [--overwrite]\n", StringComparison.Ordinal);
    message = message.Replace("  wowcrucible asset adt-height-apply <plan.json> <output.adt> [--overwrite]\n", "  wowcrucible asset adt-height-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset adt-placement-plan <input.adt> <m2|wmo> <index> <plan.json> [--position=x,y,z] [--orientation=x,y,z] [--scale=N|--scale-raw=N] [--wmo-root=root.wmo] [--overwrite]\n  wowcrucible asset adt-placement-apply <plan.json> <output.adt> [--overwrite]\n  wowcrucible asset adt-placement-multi-add-plan <input.adt> <m2|wmo> <client-path> <extracted-asset> <plan.json> --position=x,y,z [--orientation=x,y,z] [--scale=N|--scale-raw=N] [--uid=N] [--flags=N] [--doodad-set=N] [--name-set=N] [--overwrite]\n  wowcrucible asset adt-placement-multi-delete-plan <input.adt> <m2|wmo> <index> <plan.json> [--overwrite]\n  wowcrucible asset adt-placement-multi-apply <plan.json> <new-output-folder>\n  wowcrucible asset adt-placement-multi-transform-plan <input.adt> <m2|wmo> <index> <extracted-asset> <plan.json> [--position=x,y,z] [--orientation=x,y,z] [--scale=N|--scale-raw=N] [--overwrite]\n  wowcrucible asset adt-placement-multi-transform-apply <plan.json> <new-output-folder>\n  wowcrucible asset adt-placement-add-plan <input.adt> <m2|wmo> <client-path> <extracted-asset> <plan.json> --position=x,y,z [--orientation=x,y,z] [--scale=N|--scale-raw=N] [--uid=N] [--flags=N] [--doodad-set=N] [--name-set=N] [--overwrite]\n  wowcrucible asset adt-placement-delete-plan <input.adt> <m2|wmo> <index> <plan.json> [--overwrite]\n  wowcrucible asset adt-placement-lifecycle-apply <plan.json> <output.adt> [--overwrite]\n", StringComparison.Ordinal);
    message = message.Replace("map-info <file.adt|wdt|wdl> [--cells] [--format=text|json]", "map-info <file.adt|wdt|wdl> [--cells] [--placements] [--scene] [--materials --library=processed-assets [--provenance=name]] [--format=text|json]", StringComparison.Ordinal);
    if (message.Contains("wowcrucible db query", StringComparison.Ordinal))
        message += "\n\nDatabase objects:\n  wowcrucible db object-compose <database> <trigger|procedure|function|event> <name> <body.sql> [guided options] [--output=definition.sql] [--overwrite]\n  wowcrucible db objects <host> <port> <user> <database> [--type=view|trigger|procedure|function|event] [--format=text|json]\n  wowcrucible db object-show <host> <port> <user> <database> <type> <name> [--format=text|json]\n  wowcrucible db object-export <host> <port> <user> <database> <output.sql> [--overwrite]\n  wowcrucible db object-drop <host> <port> <user> <database> <type> <name> [--apply]\n  wowcrucible db object-set <host> <port> <user> <database> <type> <name> <create-definition.sql> [--apply] [--format=text|json]\n  wowcrucible db object-rollback <host> <port> <user> <database> <receipt.json> [--apply] [--format=text|json]\n  wowcrucible db view-set <host> <port> <user> <database> <name> <select.sql> [--apply]\n  wowcrucible db event-state <host> <port> <user> <database> <name> <enable|disable> [--apply]\n\nTarget-bound synchronization:\n  wowcrucible db sync-bridge <host> <port> <user> <database> <verified-audit> <bridge.json> [--include=glob]... [--maximum=N] [--overwrite]\n  wowcrucible db sync-bridge-inspect <bridge.json>\n  wowcrucible db sync-plan <host> <port> <user> <database> <verified-audit> <plan.json> [--include=glob]... [--translation=bridge.json] [--dependency-closure] [--include-removals] [--auto-remap] [--remap-start=ID] [--maximum=N] [--overwrite]\n  wowcrucible db sync-inspect <plan.json> [--sql=preview.sql] [--overwrite]\n  wowcrucible db sync-apply <host> <port> <user> <database> <plan.json> <receipt.json> [--apply] [--overwrite]\n  wowcrucible db sync-rollback <host> <port> <user> <database> <receipt.json> [--apply]\n\nobject-compose is offline and turns structured trigger, routine, or event choices into one exact validated CREATE definition; use --table/--timing/--event for triggers, --parameters/--data-access/--security plus --returns for functions, or --every/--unit/--starts/--ends versus --at for events. The output then feeds object-set or SQL Studio's exact hash-bound review. Object mutations and synchronization apply/rollback are dry-run unless --apply is explicit. object-set accepts one exact server CREATE definition for a view, trigger, procedure, function, or event; it hash-binds any existing SHOW CREATE preimage, saves a recovery receipt before DDL, restores that preimage automatically when replacement CREATE fails, and supports stale-safe object-rollback. mysql-client DELIMITER lines are not server SQL and must be removed. Guided views accept exactly one independently validated SELECT; synchronization requires a verified baseline comparison, exact primary keys, target preimage matches, and a rollback receipt. `sync-bridge` generates an editable profile bound to the audit and live target schema; blank mappings block rather than guess. `sync-bridge-inspect` verifies its offline structure and prints every decision. `--translation` supports explicit primary-row table/column renames, reviewed non-key drops, typed target defaults, and reviewed structural row expansions. Each expansion binds the complete target key and every required preimage/postimage value to a named source value or typed constant; incomplete or colliding outputs block. Its profile bytes, evidence, and target schema hash are bound into the plan. `--dependency-closure` then follows exact translated target relationships and keeps all outputs from a selected source row together. Automatic collision remapping is opt-in and every mapping/rewrite is printed for review.\n\nRead-only query batches: add --batch to execute up to 32 semicolon-delimited SELECT/SHOW/DESCRIBE/EXPLAIN statements from one SQL file. Use --batch-format=text|json for independently shaped, labeled result sets. Batches reject --write and single-result --output switches; SELECT file output is always blocked.";
        message = message.Replace("  wowcrucible db object-show <host> <port> <user> <database> <type> <name> [--format=text|json]\n", "  wowcrucible db object-show <host> <port> <user> <database> <type> <name> [--format=text|json]\n  wowcrucible db object-dependencies <host> <port> <user> <database> <type> <name> [--draft=create.sql] [--format=text|json]\n", StringComparison.Ordinal);
        message = message.Replace("The output then feeds object-set or SQL Studio's exact hash-bound review.", "The output then feeds object-set or SQL Studio's exact hash-bound review. object-dependencies reports live incoming/outgoing edges and optional draft declaration plus added/removed-edge impact. Relevant dependency edges are hash-bound and rechecked immediately before object-set applies DDL.", StringComparison.Ordinal);
        message = message.Replace("`--translation` supports explicit primary-row table/column renames, reviewed non-key drops, typed target defaults, and reviewed structural row expansions. Each expansion binds the complete target key and every required preimage/postimage value to a named source value or typed constant; incomplete or colliding outputs block.", "`--translation` supports explicit primary-row table/column renames, reviewed non-key drops, typed target defaults, structural row expansions, exact named changed-row/live-target lookups, and bounded numeric/string/map/null transforms. Every required preimage/postimage remains explicit; ambiguous lookups and incomplete or colliding outputs block. Target lookup evidence is hash-bound and rechecked under row locks before apply; arbitrary SQL/scripts and recursive lookups are refused.", StringComparison.Ordinal);
...
static bool TryParseVector3(string text, out System.Numerics.Vector3 value)
{
    value = default; var parts = text.Split(',', StringSplitOptions.TrimEntries);
    if (parts.Length != 3 || !float.TryParse(parts[0], System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var x) || !float.TryParse(parts[1], System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var y) || !float.TryParse(parts[2], System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var z) || !float.IsFinite(x) || !float.IsFinite(y) || !float.IsFinite(z)) return false;
    value = new(x, y, z); return true;
}
static bool TryParseUnsigned(string text, uint maximum, out uint value)
{
    var style = System.Globalization.NumberStyles.Integer; if (text.StartsWith("0x", StringComparison.OrdinalIgnoreCase)) { text = text[2..]; style = System.Globalization.NumberStyles.AllowHexSpecifier; }
    return uint.TryParse(text, style, System.Globalization.CultureInfo.InvariantCulture, out value) && value <= maximum;
}
static bool TryParseGridCells(string text, int gridSize, out IReadOnlyList<(int X, int Y)> cells)
{
    var parsed = new List<(int X, int Y)>(); cells = parsed; if (gridSize <= 0 || string.IsNullOrWhiteSpace(text)) return false;
    foreach (var token in text.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
    {
        var parts = token.Split(':', StringSplitOptions.TrimEntries); if (parts.Length != 2 || !int.TryParse(parts[0], out var x) || !int.TryParse(parts[1], out var y) || x < 0 || x >= gridSize || y < 0 || y >= gridSize) return false; parsed.Add((x, y));
    }
    cells = parsed.Distinct().ToArray(); return cells.Count > 0;
}
static void PrintObjectDependencyImpact(SqlDatabaseObjectImpactReport report)
{
    Console.WriteLine($"OBJECT\t{report.Type}\t{report.Database}.{report.Name}\t{(report.Exists ? "exists" : "new")}");
    Console.WriteLine($"GRAPH_SHA256\t{report.LiveGraphSha256}");
    Console.WriteLine($"DECLARATION_CHANGED\t{report.DeclarationChanged}");
    Console.WriteLine($"BEFORE_DECLARATION\t{report.BeforeDeclaration ?? "-"}");
    Console.WriteLine($"PROPOSED_DECLARATION\t{report.ProposedDeclaration}");
    foreach (var reference in report.Incoming) Console.WriteLine($"INCOMING\t{reference.Display}");
    foreach (var reference in report.OutgoingBefore) Console.WriteLine($"OUTGOING_LIVE\t{reference.Display}");
    foreach (var reference in report.OutgoingProposed) Console.WriteLine($"OUTGOING_DRAFT\t{reference.Display}");
    foreach (var reference in report.AddedOutgoing) Console.WriteLine($"ADDED\t{reference.Display}");
    foreach (var reference in report.RemovedOutgoing) Console.WriteLine($"REMOVED\t{reference.Display}");
    foreach (var finding in report.Findings) Console.WriteLine($"FINDING\t{finding}");
    Console.Error.WriteLine($"Dependency impact: {report.Incoming.Count:N0} incoming, {report.OutgoingBefore.Count:N0} live outgoing, {report.AddedOutgoing.Count:N0} added, {report.RemovedOutgoing.Count:N0} removed.");
}
