...
static int Dbc(string[] args)
{
    ...
    if (args is ["lighting", var lightingRoot, .. var lightingOptions])
    {
        ...
        if (json)
        {
            var payload = new { catalog.DbcDirectory, Counts = new { Lights = catalog.Lights.Count, Parameters = catalog.Parameters.Count, ColorBands = catalog.ColorBands.Count, FloatBands = catalog.FloatBands.Count, Skyboxes = catalog.Skyboxes.Count }, catalog.Findings, Slot = slot, Time = time, Lights = profiles.Select(value => new { value.Light, value.Profile.ParamsId, value.Profile.Parameters, value.Profile.Skybox, value.Profile.Findings, Colors = value.Profile.ColorBands.Select(band => new { band.Id, band.Index, band.Name, Value = WorldLightingService.Sample(band, time), Keys = band.Keys.Count }), Floats = value.Profile.FloatBands.Select(band => new { band.Id, band.Index, band.Name, Value = WorldLightingService.Sample(band, time), Keys = band.Keys.Count }) }) };
