...
static int Asset(string[] args)
{
    ...
    if (args.Length is 2 or 3 && args[0].Equals("preview-info", StringComparison.OrdinalIgnoreCase) && (args.Length == 2 || args[2].Equals("--all-geosets", StringComparison.OrdinalIgnoreCase)))
    {
        var mode = args.Length == 3 ? M2PreviewVisibilityMode.AllGeosets : M2PreviewVisibilityMode.BaseAppearance;
...
static async Task<int> Database(string[] args, CancellationToken cancellationToken)
{
    ...
    {
        if (args.Length < 3) return Fail("db draft-template requires creature, gameobject, or quest and an output JSON path."); var options = args[3..]; var overwrite = options.Any(option => option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)); var unknown = options.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown draft-template option: {unknown[0]}");
        object draft = args[1].ToLowerInvariant() switch
        {
            ...
            _ => throw new ArgumentException($"Unknown draft-template domain: {args[1]}")
        };
    ...
    {
        if (args.Length < 7 || args[5].StartsWith("--", StringComparison.Ordinal) || args[6].StartsWith("--", StringComparison.Ordinal)) return Fail("db content-plan requires a domain (creature, gameobject, or quest) and UTF-8 draft JSON path.");
        ...
        WorldContentWritePlan plan = args[5].ToLowerInvariant() switch
        {
            ...
            _ => throw new ArgumentException($"Unknown content-plan domain: {args[5]}")
        };
