...
static async Task<int> Database(string[] args, CancellationToken cancellationToken)
{
    ...
    if (args[0].Equals("sync-bridge-inspect", StringComparison.OrdinalIgnoreCase))
    {
        ...
        {
            var tableBlocked = string.IsNullOrWhiteSpace(table.TargetTable); if (tableBlocked) unresolved++;
            Console.WriteLine($"TABLE\t{table.SourceTable}\t{(tableBlocked ? "UNRESOLVED" : table.TargetTable)}\tobserved={table.ObservedSourceColumns?.Count ?? 0}\tkeys={string.Join(',', table.SourcePrimaryKeyColumns ?? [])}\tinsert-defaults={table.RequiresInsertDefaults}");
