...
static async Task<int> Project(string[] args, CancellationToken cancellationToken)
{
    ...
    if (args is ["race-plan", var raceProject, var raceDbc, var raceSchema, var sourceRaceText, var targetRaceText, var targetRaceName, var clientPrefix, var raceFileToken, var raceHost, var racePortText, var raceUser, var raceDatabase, .. var raceOptions])
    {
        ...
        var allowed = new[] { "--output=", "--password-env=", "--ssl=", "--male-display=", "--female-display=", "--assets=", "--provenance=" }; var unknown = raceOptions.Where(option => !allowed.Any(prefix => option.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)) && !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=json", StringComparison.OrdinalIgnoreCase) && !option.Equals("--format=text", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown race-plan option: {unknown[0]}");
        ...
        var appearance = new PlayableRaceAppearanceOptions(maleDisplay, femaleDisplay, Option(raceOptions, "--assets="), Option(raceOptions, "--provenance="));
