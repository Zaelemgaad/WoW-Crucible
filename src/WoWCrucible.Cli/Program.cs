...
static async Task<int> Database(string[] args, CancellationToken cancellationToken)
{
    ...
    {
        if (args.Length < 3) return Fail("db draft-template requires creature or gameobject and an output JSON path."); var options = args[3..]; var overwrite = options.Any(option => option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)); var unknown = options.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown draft-template option: {unknown[0]}");
    ...
    {
        if (args.Length < 7 || args[5].StartsWith("--", StringComparison.Ordinal) || args[6].StartsWith("--", StringComparison.Ordinal)) return Fail("db content-plan requires a domain (creature or gameobject) and UTF-8 draft JSON path.");
