...
static int Asset(string[] args)
{
    ...
    {
        var overwrite = brushPlanOptions.Contains("--overwrite", StringComparer.OrdinalIgnoreCase); var falloffText = Option(brushPlanOptions, "--falloff=") ?? "smooth";
        var unknown = brushPlanOptions.Where(option => !option.Equals("--overwrite", StringComparison.OrdinalIgnoreCase) && !option.StartsWith("--falloff=", StringComparison.OrdinalIgnoreCase)).ToArray(); if (unknown.Length > 0) return Fail($"Unknown asset adt-brush-plan option: {unknown[0]}");
        ...
        var plan = AdtTerrainBrushService.Plan(brushAdtPath, centerX, centerY, radius, strength, falloff); AdtTerrainBrushService.SavePlan(plan, brushPlanPath, overwrite);
        Console.Error.WriteLine($"Planned {plan.Vertices.Count:N0} MCVT vertex edit(s) across {plan.Vertices.Select(vertex => (vertex.CellX, vertex.CellY)).Distinct().Count():N0} cell(s): {Path.GetFullPath(brushPlanPath)}"); return 0;
    }
