...
static int Asset(string[] args, CancellationToken cancellationToken)
{
    ...
    {
        var json = mapOptions.Contains("--format=json", StringComparer.OrdinalIgnoreCase); var includeCells = mapOptions.Contains("--cells", StringComparer.OrdinalIgnoreCase); var includePlacements = mapOptions.Contains("--placements", StringComparer.OrdinalIgnoreCase); var includeScene = mapOptions.Contains("--scene", StringComparer.OrdinalIgnoreCase);
        ...
        if (json) Console.WriteLine(System.Text.Json.JsonSerializer.Serialize<object>(scene is null ? inspection : new { Inspection = inspection, Scene = scene }, new System.Text.Json.JsonSerializerOptions { WriteIndented = true, IncludeFields = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter() } }));
        else
        {
...
static int GroupHelp(string message, int code)
{
    ...
    message = message.Replace("map-info <file.adt|wdt|wdl> [--cells] [--format=text|json]", "map-info <file.adt|wdt|wdl> [--cells] [--placements] [--scene] [--format=text|json]", StringComparison.Ordinal);
