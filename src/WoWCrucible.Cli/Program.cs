...
static int Asset(string[] args)
{
    ...
    if (args is ["preview-info", var previewModelPath, .. var previewOptions])
    {
        ...
        foreach (var emitter in geometry.ParticleEmitters) Console.WriteLine($"PARTICLE_EMITTER\trecord={emitter.Index}\ttype={emitter.EmitterType}\t{emitter.EmitterName}\tbone={emitter.BoneIndex}\ttexture={emitter.TextureDefinitionIndex}\tblend={emitter.BlendMode}\tsheet={emitter.Columns}x{emitter.Rows}\tflags=0x{emitter.Flags:X8}\tposition={emitter.Position.X:R},{emitter.Position.Y:R},{emitter.Position.Z:R}");
...
static async Task<int> Database(string[] args, CancellationToken cancellationToken)
{
    ...
    {
        var output = Option(rawOptions, "--output="); var dbc = Option(rawOptions, "--dbc="); var json = rawOptions.Any(option => option.Equals("--format=json", StringComparison.OrdinalIgnoreCase));
