...
internal sealed class StartCenterControl : UserControl
{
    ...
    public StartCenterControl(AppSettings settings, Action editSpells, Action openDbcs, Action buildPatch, Action browseMpq, Action compareLayers, Action configurePaths)
    {
        _settings = settings; Dock = DockStyle.Fill; AutoScroll = true; BackColor = Color.FromArgb(244, 246, 249);
        ...
        guided.Controls.Add(Card("Build a client patch", "Stage edited DBC or UI files, review every path inside the MPQ, enforce a manifest policy, then create a small patch archive.", "Build Patch MPQ", buildPatch), 1, 0);
        guided.Controls.Add(Card("Inspect an existing MPQ", "Search a patch without loading its contents, inspect internal paths, and extract selected files or the whole archive.", "Browse or Extract", browseMpq), 2, 0);
        ...
        advanced.Controls.Add(Card("Compare DBC layers", "Compare base and override directories, inspect semantic changes, then promote selected fields or rows by ID.", "Open Layer Comparison", compareLayers), 1, 0);
        advanced.Controls.Add(Card("Configure the workspace", "Set server DBC, client Data, compatible Wow.exe, schema, and layered DBC paths once for faster workflows.", "Configure Paths", () => { configurePaths(); RefreshReadiness(); }), 2, 0);
    ...
    public void RefreshReadiness()
    {
        ...
        AddReadiness("Build-12340 schema", File.Exists(_settings.SchemaDefinitionPath), _settings.SchemaDefinitionPath, "Select the WotLK 3.3.5 (12340) schema XML.");
