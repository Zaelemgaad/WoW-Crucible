...
{
    private readonly TextBox _cachePath = new() { PlaceholderText = "Client Cache\\WDB\\locale\\*.wdb" };
    private readonly TextBox _definitionPath = new() { PlaceholderText = "Optional WDB.xml or wdb-definitions.xml (raw inspection works without it)" };
    ...
    public CacheTableWorkspaceView()
    {
        ...
        AddPath(paths, 0, "WDB cache", _cachePath, pickCache); AddPath(paths, 1, "Definition", _definitionPath, pickDefinition);
        _records.ItemTemplate = new FuncDataTemplate<WowCacheRecord>((record, _) => record is null ? new Grid() : BuildRecord(record));
        ...
        DragDrop.AddDragOverHandler(drop, (_, args) => { args.DragEffects = args.DataTransfer.TryGetFiles()?.Any(file => IsWdb(file.TryGetLocalPath())) == true ? DragDropEffects.Copy : DragDropEffects.None; args.Handled = true; });
        DragDrop.AddDropHandler(drop, async (_, args) => { var path = args.DataTransfer.TryGetFiles()?.Select(file => file.TryGetLocalPath()).FirstOrDefault(IsWdb); if (path is not null) await LoadAsync(path); args.Handled = true; });
        Content = drop;
    }
    ...
    public async Task LoadAsync(string path)
    {
        ...
        try
        {
            var definitionPath = _definitionPath.Text?.Trim();
            var table = await Task.Run(() =>
            {
                WowCacheTableDefinition? definition = null;
                if (!string.IsNullOrWhiteSpace(definitionPath))
                {
                    var catalog = WowCacheDefinitionCatalog.Load(definitionPath); definition = catalog.Resolve(path, WowCacheDefinitionKind.Wdb);
                }
                return WowCacheTableService.LoadWdb(path, definition);
            });
            if (request != _loadRequest) return; _table = table; Filter();
            var failures = table.Records.Count(record => record.DecodeError is not null); var remainder = table.Records.Count(record => record.UnconsumedBytes != 0);
            _summary.Text = $"{Path.GetFileName(path)} · {table.Header.Magic} · build {table.Header.Build:N0} · {table.Header.Locale} · {table.Records.Count:N0} record(s) · schema {table.Definition?.Name ?? "RAW"} · {failures:N0} decode failure(s) · {remainder:N0} remainder(s) · source SHA-256 {table.Sha256}";
            DesktopCrashLogger.Debug("CACHE", "wdb-open-success", ("path", path), ("build", table.Header.Build), ("records", table.Records.Count), ("definition", table.Definition?.SourcePath ?? "raw"), ("failures", failures));
        }
        catch (Exception exception) { if (request != _loadRequest) return; _table = null; _summary.Text = $"Cache open failed: {exception.Message}"; _detail.Text = "The source file was not changed."; DesktopCrashLogger.Log("WDB cache open failed", exception); }
    }

    private Task OpenSelectedAsync()
    {
        var path = _cachePath.Text?.Trim(); if (!IsWdb(path)) { _summary.Text = "Choose an existing .wdb file first."; return Task.CompletedTask; }
        return LoadAsync(path!);
    }

    private void Filter()
    {
        if (_table is null) return; var query = _search.Text?.Trim() ?? string.Empty;
        _records.ItemsSource = _table.Records.Where(record => query.Length == 0 || record.Id.ToString().Contains(query, StringComparison.OrdinalIgnoreCase) || record.Values.Any(value => value.Name.Contains(query, StringComparison.OrdinalIgnoreCase) || value.DisplayValue.Contains(query, StringComparison.OrdinalIgnoreCase)) || (record.DecodeError?.Contains(query, StringComparison.OrdinalIgnoreCase) ?? false)).ToArray();
    }

    private void ShowRecord()
    {
        if (_records.SelectedItem is not WowCacheRecord record) return;
        var builder = new StringBuilder(); builder.AppendLine($"RECORD {record.Id:N0} · payload {record.PayloadSize:N0} bytes · file offset 0x{record.FileOffset:X}");
    ...
    private async Task PickCacheAsync()
    {
        ...
        var files = await storage.OpenFilePickerAsync(new FilePickerOpenOptions { Title = "Open WoW client cache", AllowMultiple = false, FileTypeFilter = [new FilePickerFileType("WoW client cache") { Patterns = ["*.wdb"] }] });
    ...
    private async Task PickDefinitionAsync()
    {
        ...
        var path = files.FirstOrDefault()?.TryGetLocalPath(); if (path is null) return; _definitionPath.Text = path; if (IsWdb(_cachePath.Text)) await LoadAsync(_cachePath.Text!);
    }

    private async Task ExportAsync(string format)
    {
        if (_table is null) { _summary.Text = "Open a cache before exporting it."; return; }
        var storage = TopLevel.GetTopLevel(this)?.StorageProvider; if (storage is null) return;
        var target = await storage.SaveFilePickerAsync(new FilePickerSaveOptions { Title = $"Export decoded cache as {format.ToUpperInvariant()}", SuggestedFileName = Path.GetFileNameWithoutExtension(_table.SourcePath) + "." + format, DefaultExtension = format, FileTypeChoices = [new FilePickerFileType(format.ToUpperInvariant()) { Patterns = [$"*.{format}"] }] });
        var path = target?.TryGetLocalPath(); if (path is null) return;
        try { await Task.Run(() => WowCacheTableService.Export(_table, path, format, overwrite: true)); _summary.Text = $"Exported {_table.Records.Count:N0} record(s) atomically to {path}"; }
    ...
    private static Control BuildRecord(WowCacheRecord record)
    {
        ...
        Add(grid, record.Id.ToString("N0"), 0, "#E5EAF2", FontWeight.SemiBold); Add(grid, $"{record.PayloadSize:N0} B", 1, color); Add(grid, preview, 2); return grid;
    }
    ...
    private static bool IsWdb(string? path) => path is not null && File.Exists(path) && Path.GetExtension(path).Equals(".wdb", StringComparison.OrdinalIgnoreCase);
