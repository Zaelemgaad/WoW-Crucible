...
internal sealed class ItemCreatorView : UserControl, IDisposable
{
    ...
    private readonly NumericUpDown _entry = ItemNumber("entry", 1, 1);
    ...
    private readonly NumericUpDown _display = ItemNumber("displayid");
    ...
    private readonly NumericUpDown _itemLevel = ItemNumber("ItemLevel", 1); private readonly NumericUpDown _requiredLevel = ItemNumber("RequiredLevel");
    private readonly NumericUpDown _buy = ItemNumber("BuyPrice"); private readonly NumericUpDown _sell = ItemNumber("SellPrice");
    private readonly ComboBox _bonding = Choice((0,"No binding"),(1,"Bind on pickup"),(2,"Bind on equip"),(3,"Bind on use"),(4,"Quest item"));
    private readonly NumericUpDown _soundOverride = ItemNumber("SoundOverrideSubclass", -1);
    ...
    private readonly NumericUpDown _flags = ItemNumber("Flags"); private readonly NumericUpDown _armor = ItemNumber("armor");
    private readonly NumericUpDown _damageMin = ItemNumber("dmg_min1"); private readonly NumericUpDown _damageMax = ItemNumber("dmg_max1"); private readonly ComboBox _damageType = Choice((0,"Physical"),(1,"Holy"),(2,"Fire"),(3,"Nature"),(4,"Frost"),(5,"Shadow"),(6,"Arcane")); private readonly NumericUpDown _delay = ItemNumber("delay"); private readonly NumericUpDown _durability = ItemNumber("MaxDurability");
    private readonly NumericUpDown _itemSet = ItemNumber("itemset");
    private readonly ComboBox[] _statTypes = Enumerable.Range(0,10).Select(_ => StatChoice()).ToArray(); private readonly NumericUpDown[] _statValues = Enumerable.Range(1,10).Select(slot => ItemNumber($"stat_value{slot}")).ToArray();
    private readonly NumericUpDown[] _spellIds = Enumerable.Range(1,5).Select(slot => ItemNumber($"spellid_{slot}")).ToArray(); private readonly ComboBox[] _spellTriggers = Enumerable.Range(0,5).Select(_ => SpellTriggerChoice()).ToArray();
    private readonly NumericUpDown[] _spellCharges = Enumerable.Range(1,5).Select(slot => ItemNumber($"spellcharges_{slot}")).ToArray(); private readonly NumericUpDown[] _spellPpm = Enumerable.Range(1,5).Select(slot => ItemNumber($"spellppmRate_{slot}")).ToArray();
    private readonly NumericUpDown[] _spellCooldowns = Enumerable.Range(1,5).Select(slot => ItemNumber($"spellcooldown_{slot}",-1)).ToArray(); private readonly NumericUpDown[] _spellCategories = Enumerable.Range(1,5).Select(slot => ItemNumber($"spellcategory_{slot}")).ToArray(); private readonly NumericUpDown[] _spellCategoryCooldowns = Enumerable.Range(1,5).Select(slot => ItemNumber($"spellcategorycooldown_{slot}",-1)).ToArray();
    ...
    private readonly CheckBox _showAttachments = new() { Content = "Show native attachment points" };
    ...
    public ItemCreatorView(DesktopWorkspaceSession session)
    {
        ...
        var appearanceChoices = new StackPanel { Spacing = 5, Children = { _appearanceSkins, _appearanceFaces, _appearanceFacialHair, _appearanceHair, _appearanceSources } };
        ...
        var workspace = new ResponsiveSplitGrid(editTabs, previewTabs, 3, 2);
    ...
    public void LoadRow(IReadOnlyDictionary<string, object?> row)
    {
        ...
        var declaredStats=Math.Clamp(Int(row,"StatsCount"),0,10);
        for(var index=0;index<10;index++){var value=Int(row,$"stat_value{index+1}");var active=index<declaredStats||value!=0;Set(_statTypes[index],active?Int(row,$"stat_type{index+1}"):-1);_statValues[index].Value=value;}
    ...
    private void ResetServerOnlyDraftFields()
    {
        ...
        for(var index=0;index<10;index++){Set(_statTypes[index],-1);_statValues[index].Value=0;}
    ...
    private void RefreshSql(){ try{ var plan=Plan(); _sql.Text=plan.PreviewSql()+(plan.OmittedFields.Count==0?"":$"\n\n-- Target schema omitted: {string.Join(", ",plan.OmittedFields)}")+(plan.SelectedOmittedFields.Count==0?"":$"\n-- WRITE BLOCKED; configured values would be lost: {string.Join(", ",plan.SelectedOmittedFields)}"); _status.Text=plan.LosesSelectedSemantics?$"Write blocked: target schema cannot represent configured {string.Join(", ",plan.SelectedOmittedFields)}.":$"Plan targets {plan.Table} with {plan.Values.Count:N0} columns; {plan.OmittedFields.Count:N0} unused/default field(s) unavailable."; }catch(Exception exception){_status.Text=exception.Message;} }
    ...
    private async Task PreviewMountedModelAsync()
    {
        try
        {
            ...
            if(shoulderPlan is not null)
            {
                ...
                _modelStatus.Text=$"Mounted {_mountedItems.Count:N0} shoulder model(s) on the native left/right attachment pair. {shoulderPlan.Message} Character and shoulder faces share one depth-sorted scene.{GeosetCoverage(characterGeometry)}";
                return;
            }
            if(requestedAttachment is null)
            {
                _showAttachments.IsChecked=true;ApplyAttachmentOverlay();_modelStatus.Text="The character is loaded, but this item slot has no unambiguous automatic bind point. Choose the exact native attachment above, then press Mount selected model on character again."+GeosetCoverage(characterGeometry);return;
            }
    ...
    private async Task PreviewEquippedAsync()
    {
        try
        {
            ...
            _modelStatus.Text=$"Equipped display {_resolvedDisplay.Id:N0} · {source.Source} · wear slots {(preview.AppliedSlots.Count==0?"none":string.Join(", ",preview.AppliedSlots))} · {geosets}{(preview.MissingSlots.Count==0?string.Empty:$" · missing source slots {string.Join(", ",preview.MissingSlots)}")}{GeosetCoverage(geometry)}";
        }
    ...
    private static string GeosetCoverage(M2PreviewGeometry geometry)
    {
        var missing=geometry.GeosetSelectionFindings.Where(finding=>finding.Missing).ToArray();
        return missing.Length==0?string.Empty:$" · MISSING MODEL GEOSETS {string.Join(", ",missing.Select(finding=>$"{finding.Group}:{finding.RequestedVariant}"))}; no substitute was guessed";
    }
    ...
    private void ApplyNumericRanges(DatabaseTableCapability? table)
    {
        Apply(_entry,"entry");Apply(_display,"displayid");Apply(_itemLevel,"ItemLevel");Apply(_requiredLevel,"RequiredLevel");Apply(_buy,"BuyPrice");Apply(_sell,"SellPrice");
        Apply(_soundOverride,"SoundOverrideSubclass");Apply(_flags,"Flags");Apply(_armor,"armor");Apply(_damageMin,"dmg_min1");Apply(_damageMax,"dmg_max1");Apply(_delay,"delay");Apply(_durability,"MaxDurability");Apply(_itemSet,"itemset");
        for(var index=0;index<10;index++)Apply(_statValues[index],$"stat_value{index+1}");
        for(var index=0;index<5;index++){var slot=index+1;Apply(_spellIds[index],$"spellid_{slot}");Apply(_spellCharges[index],$"spellcharges_{slot}");Apply(_spellPpm[index],$"spellppmRate_{slot}");Apply(_spellCooldowns[index],$"spellcooldown_{slot}");Apply(_spellCategories[index],$"spellcategory_{slot}");Apply(_spellCategoryCooldowns[index],$"spellcategorycooldown_{slot}");}
        void Apply(NumericUpDown control,string field)
        {
            var contract=ItemNumericFieldCatalog.Resolve(field,table?.Find(field));var current=control.Value;
            control.Minimum=contract.Minimum;control.Maximum=contract.Maximum;control.Increment=contract.Increment;ToolTip.SetTip(control,contract.Tooltip);
            if(current is not null)control.Value=Math.Clamp(current.Value,contract.Minimum,contract.Maximum);
        }
    }
    private void SessionChanged(object? sender,EventArgs e)=>RefreshSchemaStatus(); private void RefreshSchemaStatus(){var cap=_session.DatabaseCapabilities;var table=cap?.FindTable("item_template");ApplyNumericRanges(table);_status.Text=table is not null?$"Live schema ready · {cap!.Database}.item_template · {table.Columns.Count:N0} columns · numeric controls follow the narrower SQL/runtime width":"Offline portable schema ready · numeric controls follow safe 3.3.5 runtime widths; connect Server & SQL for live schema intersection.";}
