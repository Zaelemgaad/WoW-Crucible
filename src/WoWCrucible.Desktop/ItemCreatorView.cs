...
internal sealed class ItemCreatorView : UserControl, IDisposable
{
    ...
    public ItemCreatorView(DesktopWorkspaceSession session)
    {
        ...
        var insert = AccentButton("Insert into connected database"); insert.Click += (_, _) => PrepareInsert();
        var actions = new WrapPanel { Children = { preview, export, insert, _status } };
    ...
    private void PrepareInsert(){ try{ if(!_session.DatabaseTested||_session.DatabaseProfile is null){_status.Text="Connect and verify Server & SQL before inserting.";return;} _pendingInsert=Plan(); var cancel=new Button{Content="Cancel"};var confirm=AccentButton($"Confirm insert item {(uint)(_entry.Value??0)}");cancel.Click+=(_,_)=>{_confirmation.IsVisible=false;_pendingInsert=null;};confirm.Click+=async(_,_)=>await InsertAsync(confirm);_confirmation.Child=new Grid{ColumnDefinitions=new("*,Auto,Auto"),ColumnSpacing=8,Children={new TextBlock{Text=$"Insert '{_name.Text}' into {_session.DatabaseProfile.Database}.item_template? Existing IDs are refused, never replaced.",TextWrapping=TextWrapping.Wrap,VerticalAlignment=VerticalAlignment.Center},WithColumn(cancel,1),WithColumn(confirm,2)}};_confirmation.IsVisible=true;}catch(Exception exception){_status.Text=exception.Message;} }
    private async Task InsertAsync(Button button){ if(_pendingInsert is null||_session.DatabaseProfile is null)return;try{button.IsEnabled=false;await new ItemTemplateService().InsertAsync(_session.DatabaseProfile,_pendingInsert);_status.Text="Item inserted transactionally. Restart or reload the world server as required by the detected core.";_confirmation.IsVisible=false;_pendingInsert=null;}catch(Exception exception){_status.Text=$"Insert failed: {exception.Message}";DesktopCrashLogger.Log("Item insert failed",exception);}finally{button.IsEnabled=true;} }
