...
internal sealed class WmoPreviewCanvas : Control, IDisposable
{
    ...
    private readonly Dictionary<int, PreviewBitmap> _textures = [];
    ...
    {
        ClearTextures(); foreach (var (material, texture) in textures) _textures[material] = PreviewBitmap.Create(texture); InvalidateVisual();
    }
    ...
    private sealed class WmoDrawOperation(Rect bounds, WmoPreviewGeometry geometry, IReadOnlyDictionary<int, PreviewBitmap> sourceTextures, float yaw, float pitch, float zoom, int? selectedGroup, bool wireframe, MapWmoPlacement? placement) : ICustomDrawOperation
    {
        private readonly IReadOnlyDictionary<int, PreviewBitmap> _textures = sourceTextures.ToDictionary(pair => pair.Key, pair => pair.Value.Retain());
        ...
        public void Render(ImmediateDrawingContext context)
        {
            ...
            {
                var end = Math.Min(geometry.TriangleIndices.Count, batch.TriangleStart + batch.TriangleIndexCount); _textures.TryGetValue(batch.MaterialIndex, out var texture); var blend = (uint)(batch.MaterialIndex >= 0 && batch.MaterialIndex < geometry.Materials.Count ? geometry.Materials[batch.MaterialIndex].BlendMode : 0);
                for (var offset = batch.TriangleStart; offset + 2 < end; offset += 3 * sampling)
                {
                    ...
                    faces.Add(new((a.Y + b.Y + c.Y) / 3f, batch.MaterialIndex, ia, ib, ic, ax, ay, bx, by, cx, cy, (byte)Math.Round(brightness * 15), texture?.Bitmap, blend));
                }
            }
