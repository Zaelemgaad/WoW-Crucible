...
{
    public static async Task<ProjectIdReservationOutcome> ReserveNextAsync(DesktopWorkspaceSession session, ContentIdDomain domain, string purpose, CancellationToken cancellationToken = default)
    {
        ...
        var occupancy = await new ContentIdOccupancyService().InspectAsync(domain, session.DatabaseProfile, session.DatabaseCapabilities,
            session.Settings.CoreDbcPath, session.Settings.SchemaDefinitionPath, cancellationToken: cancellationToken);
