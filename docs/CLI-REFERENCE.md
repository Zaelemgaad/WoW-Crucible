...
The desktop exposes the same engine under **Projects & shared IDs** without opening another window. It persists the active project, shows source-by-source readiness and reservation history, and allocates identities for full item copies plus guided creature, gameobject, and quest drafts before their transactional SQL is reviewed and applied. Reserving from a loaded row explicitly converts the visible decoded fields into a new INSERT variant; it never changes the primary key of an UPDATE silently. Launch directly with `WoWCrucible.Desktop-latest.exe --projects`.
