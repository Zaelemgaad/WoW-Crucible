...
Explicit build-264 shaders 0/1/2/5/6/23 and three-stage shader 3 now have bounded render plans. `preview-info` shows shader 2 as primary/environment `Opaque_AddAlpha_Alpha`, shader 5 as primary/primary additive-alpha, and shader 23 as primary/primary `Opaque_Alpha`; all three remain conservatively `exact=False`. Unimplemented routes such as edge-fade shader 21 remain visible fallbacks instead of being guessed.
