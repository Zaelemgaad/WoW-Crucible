...
The visual **Edit time band** page uses that same provider. It keeps unsaved source-bound drafts while switching among bands, adds an interpolated key at the current preview time, exposes readable clock time plus RGB hex or float values, draws responsive channel/value curves, can write the correctly named DBC into a staging folder, and makes source replacement a separate explicitly labeled backup-preserving action. Launch it directly with `WoWCrucible.Desktop-latest.exe --lighting` or search `lightmapper skybox curve` in the shared command palette; both routes stay inside the one Maps & World application screen.
