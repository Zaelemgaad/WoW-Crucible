...
`preview-info` reports every Wrath animation sequence plus validated particle-emitter type, bone, texture, blend, sprite-sheet, flags, and position. Add `--animation=N --time=MS` to resolve aliases and external `.anim` files, sample the weighted bone/camera/light pose at an exact time, and report a capped particle pose plus its first 16 deterministic sprites. This is the command-line diagnostic for the same animation and particle path used by the in-window model preview.
...
wowcrucible db table-design <host> <port> <user> <database> <table> <add|modify|rename|drop|clone|rename-table> [column] [--name=value] [--definition="complete clause"] [--first|--after=column] [--apply] [--format=text|json] --password-env=ENV_NAME [--ssl=Preferred]
...
`db table-design` is the CLI equivalent of **SQL Studio → Schema & server → Columns & table changes**. It parses exact server-normalized column clauses from `SHOW CREATE TABLE` instead of rebuilding a lossy subset, then plans add/modify/rename/drop-column, `CREATE TABLE ... LIKE` structure cloning, or table rename. Definitions may retain enum/set members, defaults, comments, generated expressions, character attributes, and other server-supported clauses; guided validation blocks statement delimiters, comments, unbalanced syntax, and injected top-level ALTER clauses. The default only prints the exact DDL and warnings. `--apply` rechecks the reviewed `SHOW CREATE TABLE` SHA-256 and refuses a stale or newly occupied target. It must atomically persist a pending artifact containing the exact preimage and reviewed SQL before sending one DDL statement to MySQL, then re-inspects the result and upgrades the same artifact with the verified after-state under the running executable's `Backups\SqlSchema` directory. A pending artifact explicitly says the DDL was not verified; it is never presented as a completed receipt. Schema evidence does not preserve data removed by `drop`, so that operation remains explicitly destructive.
