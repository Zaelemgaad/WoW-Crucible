...
`--core-source` adds a bounded, read-only scan of `src/server/scripts` and `modules` for exact player `AddItem` and `StoreNewItem` calls. Direct literals, same-file constants, and globally unique source constants produce file-and-line review evidence. Runtime-valued arguments, ambiguous constants, non-player receivers, comments, and strings are not inferred. A source call does **not** promote an item to known-path by itself because the enclosing script may be unregistered or unreachable in the connected world; Crucible keeps it in the no-path catalog until runtime registration and an in-game trigger can also be proven.
