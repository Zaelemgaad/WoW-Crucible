...
| M2/SKIN preview | Native mesh/UV, bounded discovery, SKIN material-unit and M2 texture-lookup parsing, per-submesh embedded BLP assignment, DBC-driven skin/face/facial-hair/hair selection, exact `CharHairGeosets` and `CharacterFacialHairStyles` geometry selection, decoded same-window geoset groups, naked/manual/stacked modes with one-variant-per-group manual safety, validated bone/attachment/lookup parsing with named selectable bind-point overlay, native body-atlas composition, item wear-region/equipment-geoset composition, same-scene resolved child-item mounting including strict-provenance paired shoulders, recursive M2/SKIN/BLP/WMO/ADT/WDT dependency closure, and explicit provenance | Bone-animation transforms, multi-pass shaders, animation playback, particles, export |
...
| ADT WMO placement fidelity | Native MWID→MWMO plus fixed-width MODF decoding preserves each placed instance's UID, path, finite position/orientation/extents, flags, doodad/name sets, and raw scale; same-window selection resolves provenance and applies the chosen orientation/scale to the WMO inspector | Compose multiple placed WMOs and doodad M2s together at world coordinates on terrain, then add guarded placement editing |
