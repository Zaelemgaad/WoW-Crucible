...
| ADT object placement fidelity | Native MMID→MMDX/MDDF and MWID→MWMO/MODF decoding preserves placed M2/WMO UID, path, finite position/orientation, flags, and raw scale; WMO extents plus doodad/name sets are retained. Same-window selection resolves exact provenance and previews either native M2/SKIN or WMO geometry; WMO orientation/scale are applied | Apply M2 placement transforms, compose multiple placed objects together at world coordinates on terrain, then add guarded placement editing |
