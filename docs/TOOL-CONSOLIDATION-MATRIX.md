...
| Items & sets | Schema-adaptive SQL/DBC acquisition audit with exact-ID evidence and reachable spell-creation graph; direct same-window decoded/full-SQL/Favorites/MPQ handoff; full schema copy; ItemSet clone/effects; exact ItemDisplayInfo-to-canonical-model/texture/icon dependency resolution with processed-source selection and native item-M2 preview | Equip resolved armor onto character models, finish tooltip/material/attachment fidelity, broaden script/static-analysis evidence, and complete remaining target adapters |
