...
| Server & SQL | Shared automatic WSL/local detection, complete-schema verification, native lifecycle controls, locally switchable multi-schema full-row SQL Studio, broad/exact filtering, server-side sorting, selectable paging, complete-row cloning with explicit VALUE/NULL/OMIT modes, matching read-only CLI browsing, cross-schema/binary-key favorites, primary-key-safe insert/update/delete, responsive exact-column dependency graph, complete-row dependency snapshots, file-DBC edge handoff, streaming CSV/JSONL export, dry-run transactional CSV import, confirmed query/write path, exact table DDL/index inspection and guarded index changes, process inspection/termination, permission-aware account/grant inspection, server-discovered privilege selection, reviewed parameterized account creation/password/lock/grant/revoke/drop actions, and read-only visual INNER/LEFT/RIGHT joins with unambiguous fields | Deployment-plan selection/ID remapping/rollback and remaining advanced HeidiSQL parity |
