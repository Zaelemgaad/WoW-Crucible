...
| Server & SQL | Shared automatic WSL/local detection, complete-schema verification, native lifecycle controls, paged full-row SQL Studio, primary-key-safe insert/update/delete, exact-column declared/inferred dependency counts and navigation, streaming CSV/JSONL export, dry-run transactional CSV import, confirmed query/write path, and persistent cross-table favorites | Visual join designer, indexes/users/process administration, broader domain adapters, deployment rollback, and remaining advanced HeidiSQL parity |
...
| Spells | Native same-window Avalonia effect-centric editor with staged undo/atomic save, exact AzerothCore `spell_dbc` full-record precedence comparison, 33-family related SQL-row discovery, complete SQL Studio handoff, CLI JSON/text inspection, and shared SQL/DBC ID-name pickers for spell dependencies | Visual dependency graph, effect simulation, tooltip/render fidelity, and portable multi-table spell change plans |
