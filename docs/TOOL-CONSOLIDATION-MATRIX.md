...
| Pets & companions | Complete live-schema/offline authoring for pet level stats, generated/localized name fragments, and spell-pet aura mappings; composite-key-safe insert/update, portable drafts, SQL preview/export, shared creature/spell name lookup, direct SQL/Favorites handoff, and source-shape-preserving bulk level-curve cloning/scaling with custom-column retention, missing-only or explicit exact-range policy, transactional apply, and stale target/schema refusal | Pet talent/ability graphs, family growth comparison, and in-model companion preview |
