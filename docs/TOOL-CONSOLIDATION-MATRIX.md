...
| MPQ | Native create/update/list/extract/manifest/validation/deployment; recursive provenance-aware asset closure feeding the patch builder; transaction-safe small-patch merge with byte-verified deduplication and explicit conflict policy; single-window workspace; files/folder-tree drag-and-drop; lazy breadcrumb folder browser beside global flat search; folder-recursive extraction; visible locale/anonymous states; external listfiles; atomic identity-bound compressed index cache with bounded pruning | Target-client effective-layer satisfaction checks, broader anonymous-name corpus recovery, locale-variant write/update, streaming extraction throughput, and remaining MPQEditor parity |
