...
- **Light/map visualization and band authoring — native foundation landed.** Crucible validates the complete build-12340 `Light`/`LightParams`/`LightSkybox`/`LightIntBand`/`LightFloatBand` graph, converts stored coordinates, plots sources and falloff radii without reusing legacy map imagery, samples 18 named color plus six float bands across the wrapped client day, exposes skybox paths, and jumps to any exact record. Its responsive editor owns complete 1–16-key color/float curves, readable time/RGB/value controls, interpolated key insertion, persistent drafts, staged outputs, guarded source replacement, complete-row preimages, `.bak`, reparse verification, and receipts. CLI inspection and authoring share the same providers. A rendered in-world sky dome remains a renderer fidelity stage rather than a prerequisite for safe DBC authoring.
