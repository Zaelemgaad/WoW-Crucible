...
- **Light/map visualization — native inspection and exact edit handoff landed.** Crucible now validates the complete build-12340 `Light`/`LightParams`/`LightSkybox`/`LightIntBand`/`LightFloatBand` graph, converts its stored coordinates, plots sources and falloff radii without reusing legacy map imagery, samples 18 named color plus six float bands across the wrapped client day, exposes skybox paths, and jumps to the exact record in the native DBC editor. The CLI shares the same graph. Direct graphical keyframe/curve authoring remains a later guarded editing layer; generic exact row editing already works.
