...
- **Player launcher/distribution — explicitly separate and still pending.** AmarothLauncher includes web filelists, optional linked groups, outdated managed-MPQ pruning, ZIP deployment, self-update, changelog/news, developer/player channels, and FTP changelog editing. Crucible already provides editor-side patch installation, cache clearing, local client indexing, and manifest verification, but those do not constitute a safe public player updater. A native replacement needs cryptographically authenticated channel manifests, content hashes, managed-file ownership, rollback, optional packages, resumable transport, and a hardened publishing path; this remains assigned rather than being mislabeled complete.
