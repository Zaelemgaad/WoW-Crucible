...
Use Coffee as format and workflow archaeology only. Source from a particular subproject may be studied or reused only after its individual origin and license are verified. Its strongest contribution is the breadth of map edge cases: holes, water, terrain offsets, WDT creation, reference repair, ground doodads, zone data, model collision, and import/export scripts. Crucible now clean-room creates MVER-18 terrain WDTs and edits their 64×64 MAIN presence table through exact-byte hash-bound plans while preserving unrelated flags/chunks; global-WMO WDTs remain an explicit different-lifecycle gap. The old GroundEffects utilities' useful authoring intent is also replaced natively: Crucible decodes the complete target `GroundEffectTexture` → weighted `GroundEffectDoodad` model graph, retains custom raw-ID mode, and edits only a reviewed MCLY effect field through source-hash/preimage-bound atomic transactions. Existing Wrath MH2O layers are now independently decoded across every cell/layer, mask/attribute, and height-depth/height-UV/depth-only layout; exact LiquidType and height edits retain every unrelated byte through hash/preimage-bound atomic plans. Structural water creation/removal and mask painting, zones, horizon/minimap, and repair cases remain fixtures for further native work rather than reasons to bundle the executables.
