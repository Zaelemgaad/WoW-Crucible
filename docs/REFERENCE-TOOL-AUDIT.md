...
- **BLP preview, conversion, and exact channel painting — native foundation landed.** The included BLPConverter 8.1 and Photoshop-plugin documentation established palette/DXT/alpha/mipmap and direct-authoring requirements but remain reference material rather than copied implementation. Crucible now owns a clean-room managed BLP1/BLP2 parser, palette/JPEG/raw-BGRA/DXT decode, mip validation/salvage reporting, BLP2 encode, path-preserving bulk conversion, and immutable-source RGBA/RGB/alpha brush/fill/invert editing with channel isolation, bounded undo, responsive pan/zoom, desktop/CLI parity, and atomic PNG/BLP export. Remaining work is material-layer composition, quantitative conversion-loss metrics, and complete texture dependency closure—not an external converter or image-editor runtime dependency.
