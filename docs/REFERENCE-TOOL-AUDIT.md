...
The independently implemented explicit-material coverage now also handles shader 2's base-alpha-masked environment contribution, shader 5's primary/primary additive-alpha route, and shader 23's primary/primary alpha interpolation. Real processed version-264 model pairs validate shader 2 and 23 parsing; the encountered shader-5 SKIN is orphaned, so its end-to-end claim is deliberately withheld and its route is protected by a synthetic build-264 structure test. All remain approximate until exact client alpha-output rules are reproduced, and shader 21 remains unsupported until edge fade is implemented.
