...
Database synchronization requires a baseline-compared audit whose stock and edited snapshots expose matching core identity. The live target may have additional columns, but every selected source key and changed field must exist; a target-only required INSERT column, missing table/field, occupied added key, or divergent value blocks that operation instead of guessing a translation. This supports exact same-schema and compatible additive-schema promotion today. Renamed/semantically changed fields and ID collisions across different core generations remain explicit future adapter work.
