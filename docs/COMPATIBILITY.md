...
Target comparison now proves which selected changes can be applied unchanged, which collide, and which are already present. Target-bound plans, three-way row states, SQL preview, transactional apply, exact rollback receipts, and opt-in coordinated ID remapping for selected recognized relationships are implemented. Domain labels are not dependency closure: automatic related-row selection, cross-core field adapters, and integration with the persistent project ID registry remain future work.
