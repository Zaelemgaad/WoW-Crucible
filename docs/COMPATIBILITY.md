...
The application can connect to a live MySQL/MariaDB world database and discover its complete schema through `information_schema`. Connection passwords are session-only. SQL Studio exposes every discovered table and column with primary-key-safe writes, while the guided item, creature/NPC, gameobject, and complete quest editors map only proved columns, preview/export SQL, parameterize values, and write multi-row plans transactionally. Remaining domain editors reuse the same inspection boundary.
